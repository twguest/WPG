...
def wfr_dose(wavefront):
    ...
    Power = (
        intensity * wavefront.params.photonEnergy / J2EV
    )  # 2D power mesh across face of wfr # GW
...
def plot_treyfront(
    mwf, title_fig, i_x_min, i_y_min, width, scale, X1, X2, Fineplot=None
):
    ...
    pylab.xlim(
        np.min(ya[np.where(irr_y >= imax * i_y_min)]) * S,
        np.max(ya[np.where(irr_y >= imax * i_y_min)]) * S,
    )
    ...
    pylab.xlim(
        np.min(xa[np.where(irr_x >= imax * i_x_min)]) * S,
        np.max(xa[np.where(irr_x >= imax * i_x_min)]) * S,
    )
