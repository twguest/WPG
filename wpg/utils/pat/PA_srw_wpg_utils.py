...
def PA_open_shelf(fname):
    ...
    if OS_ENV == "LINUX":
        d = shelve.open(
            "io/shelve_data/" + fname
        )  # Open the shelf doc for this instance
    else:
        d = shelve.open(
            "io\\shelve_data\\" + fname
        )  # Open the shelf doc for this instance
...
def PA_wf_data_to_csv(wf, fname, path=[], file_mode="w"):
    if OS_ENV == "LINUX":
        ...
        px_size_x = str(
            (abs(wf._srwl_wf.mesh.xStart) + abs(wf._srwl_wf.mesh.xFin))
            / abs(wf._srwl_wf.mesh.nx)
        )
        px_size_y = str(
            (abs(wf._srwl_wf.mesh.yStart) + abs(wf._srwl_wf.mesh.yFin))
            / abs(wf._srwl_wf.mesh.ny)
        )
        ...
        if file_mode == "w":
            wf_file.write(
                "Start_X,Finish_x,Start_Y,Finish_Y,Num_Px_X,Num_Px_y,Px_Size_X,Px_Size_X\n"
            )
