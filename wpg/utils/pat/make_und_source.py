...
def make_source_wavefronts():
    ...
    v = srwl_bl.srwl_uti_parse_options(
        varParam, use_sys_argv=True
    )  # Make a namespace (similar to dictionary) for the parameters
    ...
    pixels = [
        ...
        512,
        720,
    ]  # Define the pixel dimensions of the initial wave field
    ...
    for i in pixels:  # create a wavefront for every option in pixels.
        ...
        elecBeam.from_Twiss(
            _Iavg=v.ebm_i,  # twiss parameters are saved in the v object namespace
            _e=v.ebm_e,
            _sig_e=v.ebm_de,
            _emit_x=v.ebm_emx,
            _beta_x=v.ebm_betax,
        ...
        numPer = (
            v.und_len / v.und_per
        )  # Number of ID Periods (without counting for terminations
        ...
        sBx = (
            v.und_sx
        )  # Symmetry of the Horizontal field component vs Longitudinal position
        sBy = (
            v.und_sy
        )  # Symmetry of the Vertical field component vs Longitudinal position
        ...
        zcID = (
            -numPer * undPer
        )  # Longitudinal Coordinate of Undulator Center with respect to Straight Section Center [m]

        # Make the undulator
        und = srwlib.SRWLMagFldU(
            [
                srwlib.SRWLMagFldH(1, "v", By, phBy, sBy, 1),
                srwlib.SRWLMagFldH(1, "h", Bx, phBx, sBx, 1),
            ],
            undPer,
            numPer,
        )  # Ellipsoidal Undulator
        magFldCnt = srwlib.SRWLMagFldC(
            [und], ar.array("d", [xcID]), ar.array("d", [ycID]), ar.array("d", [zcID])
        ...
        zStartInteg = (
            0  # longitudinal position to start integration (effective if < zEndInteg)
        )
        ...
        srwlpy.CalcElecFieldSR(
            wf1, 0, magFldCnt, arPrecPar
        )  # calculate the electricfeild from the appended particle beam
        ...
        PA_save_tiff(
            np.log10(wf_wpg_inten), fname + "log10.tiff", ["source", "log", fname]
        )
