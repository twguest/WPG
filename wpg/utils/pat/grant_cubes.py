...
class Cube:
    ...
    def __init__(self, length, sample):
        ...
        self.qs = (
            self.make_quart()
        )  # the quarternion that defines the random rotation for the cube
        self.stamp = (
            self.make_stamp()
        )  # the 3d stamp of the cube area thats placed in the sample object
        self.loc = self.make_loc(
            self.sample
        )  # loctation of the cube in the sample space [px coordinates]
    ...
    def make_quart(self):
        ...
        while True:  # added to avoid  division by zero errors when axis is [0,0,0]
            axis = [
                rn.randint(0, 3) for i in range(3)
            ]  # random axis of rotation [111] to [333]
    ...
    def make_stamp(self):
        ...
        )  # make the initial un rotated cube
        cube_points = np.argwhere(cube_stamp == 1)  # list the points in the cube
        rotated_cube_points = np.dot(
            cube_points, self.qs.rotation_matrix
        )  # rotate the list of points about the quarternion
        ...
        for i in range(rotated_cube_points.shape[0]):
            rot_cube[
                rotated_cube_points[i, 0],
                rotated_cube_points[i, 1],
                rotated_cube_points[i, 2],
            ] = 1
        ...
        for i in range(rotated_cube_nx):
            ...
            scipym.binary_dilation(
                sample_im, structure=np.ones((2, 2)), output=sample_im
            )
    ...
    def make_loc(self, sample):
        ...
        stamp_loc_x = rn.randint(
            sample.growth_border, self.sample.nx - self.length - sample.growth_border
        )
        stamp_loc_y = rn.randint(
            sample.growth_border, self.sample.ny - self.length - sample.growth_border
        )
        stamp_loc_z = rn.randint(
            sample.growth_border, self.sample.nz - self.length - sample.growth_border
        )
...
class Sample:
    ...
    def make_array(self):
        ...
        for cube in self.cubes:
            ...
            stamp_x_start = np.round(cube.loc[0] - np.shape(cube.stamp)[0] / 2).astype(
                int
            )
            stamp_y_start = np.round(cube.loc[1] - np.shape(cube.stamp)[1] / 2).astype(
                int
            )
            stamp_z_start = np.round(cube.loc[2] - np.shape(cube.stamp)[2] / 2).astype(
                int
            )
...
tr = srwl_opt_setup_transm_from_file(
    "/opt/wpg/wpg/extensions/pat/io/sample/sample.tiff",
