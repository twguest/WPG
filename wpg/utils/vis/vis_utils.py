...
import extensions.utils.dev_utils
...
#from extensions.twpg_coherence import calc_1D_coherent_fraction as coherence
#from extensions.twpg_coherence import calc_degree_of_transverse_coherence_PCA as TDOC
#from extensions.twpg_uti_wf import coherence_log
...
#from extensions.process.get_rms import get_rms
J2EV = 6.24150934e18

def plotWavefront(wfr, item = "both", save = False, savedir = "", gsm = False,
                  lims = None):
    ...
    if item == "both":
        ...
        im1 = ax1.imshow(II, extent = [x1 * 1e6, x2 * 1e6, y1 * 1e6, y2 * 1e6],
                         cmap = cm.gray)
        ...
        im2 = ax2.imshow(phase, extent = [x1 * 1e6, x2 * 1e6, y1 * 1e6, y2 * 1e6],
                         cmap = cm.phase)
        divider = make_axes_locatable(ax2)
        cax = divider.append_axes('right', size='5%', pad=0.05)
        fig.colorbar(im2, cax=cax, orientation='vertical')
        
        cb2 = fig.colorbar(im2, cax=cax, orientation='vertical')#, ticks=[-np.pi, -np.pi/2, 0, np.pi/2, np.pi])
...
def plot3d(wfr, nplots = 1):
    ...
    surf = ax.plot_surface(xv, yv, ii, cmap='hot',
                           linewidth=0, antialiased=False, facecolors=cm.hsv(norm(phase)))
    ...
    cb = fig.colorbar(m, shrink = 0.5, aspect = 5, ticks=[-2*np.pi,-np.pi,0, np.pi, 2*np.pi])
...
def plotPhaseSpace(phasespace, title = "", save = False, outdir = "", fileid = ""):
    ...
    ax1 = sns.jointplot(x,y,kind = 'hex', label = fileid, xlim = [-2000, 2000], ylim = [-20, 20] )
    ax1.set_axis_labels("Horizontal Position ($\mu$m)","Veritcal Position ($\mu$m)", fontsize = 22)
    
    ax1.ax_joint.tick_params(axis='both', which='major', labelsize=16)

    ax2 = sns.jointplot(x_,y_, kind = 'kde', xlim = [-100, 100], ylim = [-6,6])
    ax2.set_axis_labels("Horizontal Trajectory ($\mu$rad)", "Vertical Trajectory ($\mu$rad)", fontsize = 22)
    ...
    ax3.set_axis_labels("Horizontal Position ($\mu$m)", "Horizontal Trajectory ($\mu$rad)", fontsize = 22)
    ...
    ax4.set_axis_labels("Vertical Position ($\mu$m)", "Vertical Trajectory ($\mu$rad)", fontsize = 22)
...
def plotModess(wfr):
    ...
    no_mode = 0
    for mode in beamline.wfr.modes:
        eField = mode.data.arrEhor
        intensity = abs(eField[:,:,0,0])
        
        x_axis = np.linspace(mode.params.Mesh.xMin,
                             mode.params.Mesh.xMax,
                             mode.params.Mesh.nx)
        
        y_axis = np.linspace(mode.params.Mesh.yMin,
                             mode.params.Mesh.yMax,
                             mode.params.Mesh.ny)
    
        sns.heatmap(intensity, vmin = 0)
        plt.title(str(no_mode))
...
def rmsPlot(wfrs, axis = 'x', save = False, savedir = ""):
    ...
    for wfr in wfrs:
        ...
        ax.plot(axis, II[II.shape[0]//2,:], color= cmap[itr], label = "$\sigma_x$ = " + "{:.2E}".format(get_rms(wfr)[4]) + " $\mu$m")
        ax.plot(axis, II[:,II.shape[1]//2,], color= cmap[itr], label = "$\sigma_y$ = " + "{:.2E}".format(get_rms(wfr)[3]) + " $\mu$m")
        itr += 1
