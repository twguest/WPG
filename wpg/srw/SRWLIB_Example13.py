...
strSpecOutFileName0 = (
    "ex13_res_spec.dat"  # file name for output SR spectrum vs photon energy data
)
...
LeffBM = 4.0  # Magnet length [m] (exaggerated, to avoid observinng eventual "edge radiation")
...
eBeam.partStatMom1.x = 0.0  # Initial horizontal position of central trajectory [m]
eBeam.partStatMom1.y = 0.0  # Initial vertical position of central trajectory [m]
eBeam.partStatMom1.z = 0.0  # Initial longitudinal position of central trajectory [m]
eBeam.partStatMom1.xp = 0.0  # Initial horizontal angle of central trajectory [rad]
eBeam.partStatMom1.yp = 0.0  # Initial vertical angle of central trajectory [rad]
...
wfrSp.allocate(
    500, 1, 1
)  # Numbers of points vs photon energy, horizontal and vertical positions (the last two will be modified in the process of calculation)
wfrSp.mesh.zStart = 5.0  # Longitudinal position for initial wavefront [m]

wfrSp.mesh.eStart = 0.1  # Initial photon energy [eV]
wfrSp.mesh.eFin = 10000.0  # Final photon energy [eV]

wfrSp.mesh.xStart = 0.0  # Initial horizontal position [m]
wfrSp.mesh.xFin = wfrSp.mesh.xStart  # Final horizontal position [m]
wfrSp.mesh.yStart = 0.0  # Initial vertical position [m]
wfrSp.mesh.yFin = 0.0  # Final vertical position [m]
...
distSrcLens = 5.0  # Distance from geometrical source point to lens [m]
...
srwl.CalcIntFromElecField(
    arSp, wfrSp, 6, 0, 0, meshSp.eStart, 0, 0
)  # Extracting intensity vs photon energy
srwl_uti_save_intens_ascii(
    arSp, meshSp, os.path.join(os.getcwd(), strExDataFolderName, strSpecOutFileName0)
)
print("done in", round(time.time() - t0), "s")
...
)  # "Flat" array to take 2D intensity data (vs X & Y)
srwl.CalcIntFromElecField(
    arI0, wfr, 6, 0, 3, mesh0.eStart, 0, 0
)  # Extracting intensity vs horizontal and vertical positions
srwl_uti_save_intens_ascii(
    arI0, mesh0, os.path.join(os.getcwd(), strExDataFolderName, strIntOutFileName0)
)
print("done in", round(time.time() - t0), "s")
...
print(
    "   Extracting intensity from calculated electric field and saving it to file(s) ... ",
    end="",
)
t0 = time.time()
...
)  # "Flat" array to take 2D single-electron intensity data (vs X & Y)
srwl.CalcIntFromElecField(
    arI1s, wfr, 6, 0, 3, mesh1.eStart, 0, 0
)  # Extracting single-electron intensity vs X & Y
srwl_uti_save_intens_ascii(
    arI1s, mesh1, os.path.join(os.getcwd(), strExDataFolderName, strIntOutFileName1)
)

arI1m = deepcopy(
    arI1s
)  # "Flat" array to take 2D multi-electron intensity data (vs X & Y)
srwl.CalcIntFromElecField(
    arI1m, wfr, 6, 1, 3, mesh1.eStart, 0, 0
)  # Calculating multi-electron intensity vs X & Y using convolution method (assuming it to be valid!)
srwl_uti_save_intens_ascii(
    arI1m, mesh1, os.path.join(os.getcwd(), strExDataFolderName, strIntOutFileName2)
)
...
uti_plot1d(
    arSp,
    [meshSp.eStart, meshSp.eFin, meshSp.ne],
    labels=("Photon Energy", "Spectral Intensity", "On-Axis SR Intensity Spectrum"),
    units=["eV", "ph/s/0.1%bw/mm^2"],
)
unitsIntPlot = ["m", "m", "ph/s/.1%bw/mm^2"]
uti_plot2d1d(
    arI0,
    [mesh0.xStart, mesh0.xFin, mesh0.nx],
    [mesh0.yStart, mesh0.yFin, mesh0.ny],
    labels=("Horizontal position", "Vertical position", "Intensity Before Lens"),
    units=unitsIntPlot,
)
uti_plot2d1d(
    arI1s,
    [mesh1.xStart, mesh1.xFin, mesh1.nx],
    [mesh1.yStart, mesh1.yFin, mesh1.ny],
    labels=(
        "Horizontal position",
        "Vertical position",
        "Single-E Intensity in Image Plane",
    ),
    units=unitsIntPlot,
)
uti_plot2d1d(
    ...
    labels=(
        "Horizontal position",
        "Vertical position",
        "Multi-E Intensity in Image Plane",
    ),
    units=unitsIntPlot,
)
