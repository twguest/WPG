...
class SRWLBeamline(object):
    ...
    def calc_el_trj(self, _ctst, _ctfi, _np=50000, _mag_type=1, _fname=""):
        ...
        if self.eBeam is None:
            Exception("Electron Beam structure is not defined")

        if _mag_type == 1:
            if self.mag_approx is None:
                Exception("Approximate Magnetic Field is not defined")
        elif _mag_type == 2:
            if self.mag is None:
                Exception("Magnetic Field is not defined")
        else:
            Exception("Incorrect Magnetic Field type identificator")
    ...
    def calc_arb_spec_me(
        ...
        if _mag == 1:
            ...
            if self.mag is None:
                Exception("Accurate Magnetic Field is not defined")
        ...
        if hasattr(_v, "mag_mdir"):
            ...
            if len(_v.mag_mdir) > 0:
                if hasattr(_v, "mag_ifn"):
                    magPath = os.path.join(
                        os.getcwd(), self.dir_main, _v.mag_mdir, _v.mag_ifn
                    )
                    ...
                    if os.path.exists(magPath):
                        ...
                        self.set_mag_tab(  # setup magnet from tabulated magnetic field data file
                            _fpath=magPath, _zc=_v.mag_zc, _interp_ord=3
                        )
