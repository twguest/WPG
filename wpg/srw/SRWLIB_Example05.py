...
AuxSaveTrajData(
    partTraj, os.path.join(os.getcwd(), strExDataFolderName, strTrajOutFileName)
)
print("done")
...
wfr1.mesh.zStart = 300.0  # Longitudinal Position [m] at which SR has to be calculated
...
wfr2.mesh.zStart = 300.0  # Longitudinal Position [m] at which SR has to be calculated
...
for i in range(partTraj.np):
    partTraj.arY[i] *= 1000
uti_plot1d(
    partTraj.arY, ctMesh, ["ct [m]", "Vertical Position [mm]", "Electron Trajectory"]
)
