...
def srwl_und_cor_fld_int(
    ...
    invDistBwKicks = 1.0 / _dist_bw_kicks
    ...
    for iy in range(_mag3d.ny):
        ...
        for ix in range(_mag3d.nx):
            ...
            if arBxOrig is not None:
                ...
                auxI2X = -(
                    arBxInt[izEr] - arBxInt[izBr] - auxI1X_Br * (izEr - izBr) * zStep
                )  # [T.m^2]
            ...
            if arByOrig is not None:
                ...
                auxI2Y = -(
                    arByInt[izEr] - arByInt[izBr] - auxI1Y_Br * (izEr - izBr) * zStep
                )  # [T.m^2]
...
# ****************************************************************************
def srwl_und_fld_add_const(
    _mag3d, _zcMesh=0, _zc=0, _zRange=0, _bx=0, _by=0, _bz=0, _dupl=False
):
    """
    ...
    nz = _mag3d.nz
    zStep = _mag3d.rz / (nz - 1)
    inv_zStep = 0.0 if (zStep == 0.0) else 1.0 / zStep
    ...
    if _zRange > 0:
        ...
        if izAddTestB <= izAddTestE:
            if (izAddB < izAddTestB) and (izAddTestB <= izAddE):
                izAddB = izAddTestB
            if (izAddB <= izAddTestE) and (izAddTestE < izAddE):
                izAddE = izAddTestE
    ...
    for iz in range(izStartRes, izEndRes + 1):
        ...
        for iy in range(ny):
            ...
            for ix in range(nx):
                ...
                if arBxOrig is not None:
                    arBxRes[i0] = arBxOrig[i]
                if arByOrig is not None:
                    arByRes[i0] = arByOrig[i]
                if arBzOrig is not None:
                    arBzRes[i0] = arBzOrig[i]
...
def srwl_und_find_cen_len(_mag3d, relThrB=0.8):
    ...
    if (_mag3d is None) or ((_mag3d.arBx is None) and (_mag3d.arBy is None)):
        raise Exception(
            "Incorrect definition of the input tabulated undulator magnetic field"
        )

    if (relThrB <= 0.0) or (relThrB >= 1.0):
        raise Exception(
            "relative threshold to be used with respect to peak field should be larger than 0 and less than 1"
        )
    ...
    if nz <= 1:
        raise Exception(
            "1D magnetic field with more than one grid point vs longitudinal position is expected"
        )
    if (nx > 1) or (ny > 1):
        raise Exception(
            "1D magnetic field with more than one grid point vs longitudinal position and one point vs horizontal and vertical position is expected"
        )

    BxIsDefined = False
    if _mag3d.arBx is not None:
        if len(_mag3d.arBx) == nz:
            BxIsDefined = True

    ByIsDefined = False
    if _mag3d.arBy is not None:
        if len(_mag3d.arBy) == nz:
            ByIsDefined = True

    if (BxIsDefined == False) and (ByIsDefined == False):
        raise Exception(
            "1D magnetic field data (vertical or horizontal component) are not defined"
        )
    ...
    if (absBxMax <= 0.0) and (absByMax <= 0.0):
        raise Exception(
            "Non-zero 1D magnetic field data (vertical or horizontal component) are not defined"
        )
...
def srwl_und_fld_1d_mis(
    ...
    if (
        (_mag3d is None)
        or ((_mag3d.arBx is None) and (_mag3d.arBy is None))
        or (_per <= 0.0)
    ):
        raise Exception(
            "Incorrect definition of the input tabulated undulator magnetic field"
    ...
    if BorigIsDefined == False:
        raise Exception(
            "1D magnetic field data (vertical or horizontal component) is not defined"
        )
...
# ****************************************************************************
def srwl_uti_und_gen_file_names_for_conv(
    _ifln,
    _ofn_core=None,
    _pref_gap=None,
    _pref_mode=None,
    ...
    numGapPref = len(prefGap[0])
    numGapPost = len(prefGap[1])
    numModePref = len(prefMode[0])
    numModePost = len(prefMode[1])
    numPhasePref = len(prefPhase[0])
    numPhasePost = len(prefPhase[1])
    ...
    for root, dirs, files in os.walk(_ifln):
        for fnOrig in files:
            ...
            for j in range(numGapPref):
                ...
                if iGapStart0 >= 0:
                    iGapStart = iGapStart0 + len(prefGap[0][j])
                # print(iGapStart0)
                if iGapStart >= 0:
                    break
            if (iGapStart < 0) or (iGapStart > lenFnOrigMi1):
                raise Exception(strErrNoGapFound)

            for j in range(numGapPost):
                iGapEnd = fnOrig.find(prefGap[1][j], iGapStart + 1)
                if iGapEnd >= 0:
                    break
            if (iGapEnd < 0) or (iGapEnd > lenFnOrigMi1):
                raise Exception(strErrNoGapFound)
            ...
            if (iTestModeIsThere >= 0) and (iTestModeIsThere < 3):
                ...
                for j in range(numModePref):
                    ...
                    if iModeStart0 >= 0:
                        iModeStart = iModeStart0 + len(prefMode[0][j])
                    if iModeStart >= 0:
                        break
                ...
                if (iModeStart < 0) or (iModeStart > lenFnOrigMi1):
                    raise Exception(strErrNoModeFound)
                ...
                for j in range(numModePost):
                    ...
                    if iModeEnd >= 0:
                        break
                if (iModeEnd < 0) or (iModeEnd > lenFnOrigMi1):
                    raise Exception(strErrNoModeFound)
            ...
            if (iTestPhaseIsThere >= 0) and (iTestPhaseIsThere < 3):
                ...
                for j in range(numPhasePref):
                    ...
                    if iPhaseStart0 >= 0:
                        iPhaseStart = iPhaseStart0 + len(prefPhase[0][j])
                    if iPhaseStart >= 0:
                        break
                if (iPhaseStart < 0) or (iPhaseStart > lenFnOrigMi1):
                    raise Exception(strErrNoPhaseFound)

                for j in range(numPhasePost):
                    iPhaseEnd = fnOrig.find(prefPhase[1][j], iPhaseStart + 1)
                    if iPhaseEnd >= 0:
                        break
                if (iPhaseEnd < 0) or (iPhaseEnd > lenFnOrigMi1):
                    raise Exception(strErrNoPhaseFound)

                phase = float(fnOrig[iPhaseStart:iPhaseEnd])

            fnRes = (
                resFileNameCore + "_g" + str(gap)
            )  # + '_m' + modeFin + '_ph' + str(phase)
...
def srwl_uti_und_conv_meas_fld(_ifn):
    ...
    fldCnt = SRWLMagFldC(
        SRWLMagFld3D(arBX, arBY, arBZ, 1, 1, zNp, 0.0, 0.0, zFi - zSt),
        0.0,
        0.0,
        0.5 * (zSt + zFi),
    )
...
def srwl_uti_und_proc_one_fld(_opt):
    ...
    if fldCnt is None:
        fldCnt = srwl_uti_read_mag_fld_3d(_opt.ifn)
    ...
    if (_opt.dbx != 0.0) or (_opt.dby != 0.0) or (_opt.dbz != 0.0):
        srwl_und_fld_add_const(
            fld3d, fldCnt.arZc[0], _opt.dzc, _opt.dzr, _opt.dbx, _opt.dby, _opt.dbz
        )
        # print(2)
    ...
    if _opt.rz > 0.0:  # Cutting the field
        if zk >= 1.0e22:
            zk, auxUndLen = srwl_und_find_cen_len(fld3d)
    ...
    if len(_opt.ofn) > 0:
        ...
        fld3d.save_ascii(_opt.ofn, 0.0, 0.0, zc)
        # print('zc=', zc)
        # print(5)

    if len(_opt.otfn) > 0:
        # z0 = opt.z0t if(opt.z0t != 1.e+23) else fldCnt.arZc[0] - 0.5*fld3d.rz
        z0 = _opt.z0t if (_opt.z0t != 1.0e23) else zc - 0.5 * fld3d.rz
        ...
        elec = SRWLParticle(
            ...
            _xp=_opt.xp0t,
            _yp=_opt.yp0t,
            _gamma=_opt.elen / 0.51099890221e-03,
        )
...
if __name__ == "__main__":
    ...
    p.add_option(
        ...
        action="store_true",
        default=False,
        help="correct 1st and 2nd field integrals of tabulated undulator magnetic field",
    )
    p.add_option(
        "--conv",
        dest="do_conv_fld",
        action="store_true",
        default=False,
        help="convert magnetic measurement data file to SRW ASCII format",
    )
    ...
    p.add_option(
        "--ifn", dest="ifn", type="string", default="", help="input field file name"
    )
    p.add_option(
        "--ofn", dest="ofn", type="string", default="", help="output field file name"
    )
    p.add_option(
        "--itfn",
        dest="itfn",
        type="string",
        default="",
        help="input trajectory file name",
    )
    p.add_option(
        "--otfn",
        dest="otfn",
    ...
    p.add_option(
        ...
        default=1.0e23,
        metavar="NUMBER",
        help="center longitudinal coordinate of the output magnetic field [m]",
    )  # OC22062017
    ...
    p.add_option(
        ...
        default=2.8,
        metavar="NUMBER",
        help="distance between correcting kicks [m]",
    )
    p.add_option(
        "--rmsk",
        dest="rmsk",
        ...
        default=0.05,
        metavar="NUMBER",
        help="RMS length of the correcting kicks [m]",
    )
    ...
    p.add_option(
        ...
        default=0.0,
        metavar="NUMBER",
        help="new longitudinal range of magnetic field after truncation [m] (effective if > 0; the center field will be taken from zk)",
    )
    ...
    p.add_option(
        ...
        default=1.0e23,
        metavar="NUMBER",
        help="longitudinal position of the correcting kicks [m]",
    )
    p.add_option(
        "--z0t",
        dest="z0t",
        ...
        default=1.0e23,
        metavar="NUMBER",
        help="initial longitudinal position for trajectory calculation [m]",
    )
    p.add_option(
        "--x0t",
        dest="x0t",
        ...
        default=0.0,
        metavar="NUMBER",
        help="initial horizontal position for trajectory calculation [m]",
    )
    p.add_option(
        "--y0t",
        dest="y0t",
        ...
        default=0.0,
        metavar="NUMBER",
        help="initial vertical position for trajectory calculation [m]",
    )
    p.add_option(
        "--xp0t",
        dest="xp0t",
        ...
        default=0.0,
        metavar="NUMBER",
        help="initial horizontal angle for trajectory calculation [m]",
    )
    p.add_option(
        "--yp0t",
        dest="yp0t",
        ...
        default=0.0,
        metavar="NUMBER",
        help="initial vertical angle for trajectory calculation [m]",
    )
    p.add_option(
        "--elen",
        dest="elen",
    ...
    p.add_option(
        ...
        default=0.0,
        metavar="NUMBER",
        help="horizontal magnetic field component to add [T]",
    )
    p.add_option(
        "--dby",
        dest="dby",
        ...
        default=0.0,
        metavar="NUMBER",
        help="vertical magnetic field component to add [T]",
    )
    p.add_option(
        "--dbz",
        dest="dbz",
        ...
        default=0.0,
        metavar="NUMBER",
        help="longitudinal magnetic field component to add [T]",
    )
    p.add_option(
        "--dzc",
        dest="dzc",
        ...
        default=0.0,
        metavar="NUMBER",
        help="center longitudinal position for adding constant magnetic field [m]",
    )
    p.add_option(
        "--dzr",
        dest="dzr",
        ...
        default=0.0,
        metavar="NUMBER",
        help="range of longitudinal position within which to add constant magnetic field [m]",
    )
    ...
    p.add_option(
        "--cnvm",
        dest="do_conv_many_files",
        action="store_true",
        default=False,
        help="convert magnetic measurement data from multiple files in a folder to SRW ASCII format, apply field integral correction and save to another folder, and save a summary file to the same folder",
    )
    p.add_option(
        "--ofnc",
        dest="out_file_name_core",
        type="string",
        default="",
        help="output file name core",
    )
    p.add_option(
        "--prfg",
        dest="pref_gap",
        type="string",
        default="",
        help="string of coma-separated prefixes for recognizing Gap values in the input magn. meas. data file names (it overrides the default definitions)",
    )
    p.add_option(
        "--psfg",
        dest="postf_gap",
    ...
    p.add_option(
        ...
        type="string",
        default="",
        help="string of coma-separated prefixes for recognizing Phase Mode values in the input magn. meas. data file names (it overrides the default definitions)",
    )
    p.add_option(
        "--psfm",
        dest="postf_mode",
    ...
    p.add_option(
        ...
        type="string",
        default="",
        help="string of coma-separated prefixes for recognizing Phase values in the input magn. meas. data file names (it overrides the default definitions)",
    )
    p.add_option(
        "--psfp",
        dest="postf_phase",
    ...
    p.add_option(
        ...
        type="string",
        default="gmp",
        help="order of gap-mode-phase in the input file names",
    )
    p.add_option(
        "--ipm",
        dest="in_phase_mode",
    ...
    if opt.do_conv_many_files:
        ...
        lstPrefPostfPhase = uti_parse.str_to_pair_of_lists(
            opt.pref_phase, opt.postf_phase
        )
        lstModeDict = uti_parse.str_to_list_of_pairs(
            opt.in_phase_mode, opt.out_phase_mode
        )
        # print(lstPrefPostfGap, lstPrefPostfMode, lstPrefPostfPhase, lstModeDict)

        lstParamFileNames = srwl_uti_und_gen_file_names_for_conv(
            opt.ifn,
            _ofn_core=opt.out_file_name_core,
            _pref_gap=lstPrefPostfGap,
            _pref_mode=lstPrefPostfMode,
            _pref_phase=lstPrefPostfPhase,
            _dict_mode=lstModeDict,
            _order_gmp=opt.ord_gmp_inp,
        )
