...
strOffKickTrajOutFileName = (
    "ex01_res_traj_kick_off.dat"  # file name for output trajectory data
)
...
arPrecTrajFromKickM = [
    1
]  # Precision parameters for Trajectory calculation from Kick-Matrices:
...
magFldCnt.arMagFld[
    0
].interp = 3  # Magnetic Field Interpolation Method, to be entered into 3D field structures below (to be used e.g. for trajectory calculation):
...
partTraj.partInitCond.z = (
    -0.5 * magFldCnt.arMagFld[0].rz
)  # Initial Longitudinal Coordinate (before ID)
...
srwl.CalcPartTraj(
    partTraj, magFldCnt, arPrecTrajFromField
)  # First Calculate Central Trajectory
partTraj.save_ascii(
    os.path.join(os.getcwd(), strExDataFolderName, strCenTrajOutFileName)
)

partTraj.partInitCond.x = newInitCondX
partTraj.partInitCond.y = newInitCondY
srwl.CalcPartTrajFromKickMatr(
    partTraj, kickMatr, arPrecTrajFromKickM
)  # Calculate Off-Axis Trajectory using Kick-Matrix method
partTraj.save_ascii(
    os.path.join(os.getcwd(), strExDataFolderName, strOffKickTrajOutFileName)
)

srwl.CalcPartTraj(
    partTraj, magFldCnt, arPrecTrajFromField
)  # Calculate same Off-Axis Trajectory by Runge-Kutta integration in 3D Magnetic Field
partTraj.save_ascii(
    os.path.join(os.getcwd(), strExDataFolderName, strOffTrajOutFileName)
)
...
# **********************Plotting results
print(
    "   Plotting the results (close all graph windows to proceed with the script execution) ... ",
    end="",
)
