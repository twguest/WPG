...
print("!!!!!Under testing!!!!!")
print(
    "Simulating emission and propagation of undulator radiation (UR) wavefront through a microscopy beamline with a secondary source aperture and ellopsoidal K-B mirrors used for final focusing"
)
print("")
print(
    "First, single-electron UR wavefront at a fixed photon energy is calculated and propagated through the optical scheme. ",
    end="",
)
print(
...
elecBeam.partStatMom1.y = 0.0  # -0.00025
elecBeam.partStatMom1.z = (
    0.0  # -0.5*undPer*(numPer + 4) #Initial Longitudinal Coordinate (set before the ID)
)
elecBeam.partStatMom1.xp = 0.0  # Initial Relative Transverse Velocities
elecBeam.partStatMom1.yp = 0.0
elecBeam.partStatMom1.gamma = 3.0 / 0.51099890221e-03  # Relative Energy
...
sampFactNxNyForProp = (
    0.2  # 0.6 #sampling factor for adjusting nx, ny (effective if > 0)
)
...
Drift_Slits_HFM = SRWLOptD(
    22.0
)  # Drift from first Slits to Horizontally-Focusing Mirror (HFM)

HFM = SRWLOptL(9.92727)  # HFM as Thin Lens

Drift_HFM_SSA = SRWLOptD(13.0)  # Drift from HFM to Secondary Source Aperture (SSA)

SSA = SRWLOptA("r", "a", 30e-06, 10e-03)

Drift_SSA_VKB = SRWLOptD(
    11.0
)  # Drift from SSA to Center of Vertically Focusing K-B Mirror (VKB)
...
TwoSlit_Drift = SRWLOptD(20.0)  # Drift space after 2 slits
...
VKB = SRWLOptMirEl(
    ...
    _y=0,
    _treat_in_out=1,
)  # VKB Ellipsoidal Mirror

Drift_VKB_HKB = SRWLOptD(
    0.5
)  # Distance between centers of Vertically and Horizontally focusing K-B mirrors
...
if (twoSlitPlane == "h") or (twoSlitPlane == "v"):
    optBL = SRWLOptC(
        [
            ...
            TwoSlit_Ob,
            TwoSlit_Drift,
        ],  #   ApKB,   VKB,   Drift_VKB_HKB,   HKB,   Drift_HKB_Sample],
        [
            ppDrift_Slits_HFM,
            ppHFM,
            ppDrift_HFM_SSA,
    ...
    )  # , ppApKB, ppVKB, ppDrift_VKB_HKB, ppHKB, ppDrift_HKB_Sample, ppFinal])
else:
    optBL = SRWLOptC(
        [
            Drift_Slits_HFM,
...
if srwl_uti_proc_is_master():
    ...
    srwl_uti_save_intens_ascii(
        arI,
        wfr.mesh,
        os.path.join(os.getcwd(), strExDataFolderName, strIntSE_OutFileName),
    )
    print("done")
    ...
    srwl_uti_save_intens_ascii(
        arI1,
        wfr.mesh,
        os.path.join(os.getcwd(), strExDataFolderName, strIntPropSE_OutFileName),
    )
    print("done")
...
radStokesProp = srwl_wfr_emit_prop_multi_e(
    ...
    nMacroElecSavePer,
    os.path.join(os.getcwd(), strExDataFolderName, strIntPropME_OutFileName),
    sampFactNxNyForProp,
    optBL,
)
print("done")
