...
def AnalyticEst(PhotonEnergy, WaistPozition, Waist, Dist):
    ...
    for l in range(len(Dist)):
        wRMSan.append(
            1
            * Waist
            * sqrt(1 + (Lam * (Dist[l] - WaistPozition) / 4 / 3.1415 / Waist ** 2) ** 2)
        )
    return wRMSan
...
def Container(DriftLength, f_x, f_y):
    ...
    )  # note that I use sel-adjust for Num grids
...
firstHorAp = 20.0e-03  # First Aperture [m]
firstVertAp = 30.0e-03  # [m]
...
f_x = 3e0  # focusing strength, m in X
f_y = 4e0  # focusing strength, m in Y
...
NumSteps = int(
    (TotalLength - InitialDist) / StepSize
)  # Number of steps to sample RMS x/y after the lens
...
header = "{:3s} {:8s} {:8s} {:2s} {:2s} {:13s} {:13s}".format(
...
for j in range(NumSteps):
    ...
    DependArg = (
        3  # 0 - vs e, 1 - vs x, 2 - vs y, 3- vs x&y, 4-vs x&e, 5-vs y&e, 6-vs x&y&e
    )
    # plotNum = 1000
    ...
    if InitialDist == 0.0:  # plot initial intensity at 0
        ...
        intensitiesToPlot["mesh_x"].append(
            deepcopy([wfrP.mesh.xStart, wfrP.mesh.xFin, wfrP.mesh.nx])
        )
    ...
    arII = array(
        "f", [0] * wfrP.mesh.nx * wfrP.mesh.ny
    )  # "flat" array to take 2D intensity data
    arIE = array("f", [0] * wfrP.mesh.nx * wfrP.mesh.ny)
    srwl.CalcIntFromElecField(
        arII, wfrP, Polar, Intens, DependArg, wfrP.mesh.eStart, 0, 0
    )
    ...
    if (
        abs(InitialDist - 3.0) < 1e-10 or abs(InitialDist - 3.9) < 1e-10
    ):  # plot at these distances
    ...
    for i in range(wfrP.mesh.nx):
        x.append(
            (i - wfrP.mesh.nx / 2.0)
            / wfrP.mesh.nx
            * (wfrP.mesh.xFin - wfrP.mesh.xStart)
        )
    ...
    for i in range(wfrP.mesh.ny):
        y.append(
            (i - wfrP.mesh.ny / 2.0)
            / wfrP.mesh.ny
            * (wfrP.mesh.yFin - wfrP.mesh.yStart)
        )
    ...
    data_row = "{:3.1f} {:8.6f} {:8.6f} {:2d} {:2d} {:12.10f} {:12.10f}".format(
        InitialDist,
        xRMS[j],
        yRMS[j],
        wfrP.mesh.nx,
...
for m in range(len(s)):
    Wx = (WRx[m][0][0] * qx0 + WRx[m][0][1]) / (
        WRx[m][1][0] * qx0 + WRx[m][1][1]
    )  # MatrixMultiply(WR,qxP)
    RMSbx = sqrt(1.0 / (-1.0 / Wx).imag / 3.1415 * Lam) * 2.35
    Wthx.append(RMSbx)
    Wy = (WRy[m][0][0] * qy0 + WRy[m][0][1]) / (
        WRy[m][1][0] * qy0 + WRy[m][1][1]
    )  # MatrixMultiply(WR,qxP)
...
for i in range(len(intensitiesToPlot["intensity"])):
    srwl_uti_save_intens_ascii(
        ...
        "{}/intensity_{:.1f}m.dat".format(
            example_folder, intensitiesToPlot["distance"][i]
        ),
        0,
