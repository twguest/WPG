...
strFldOutFileName = (
    "ex02_res_fld.dat"  # file name for output (tabulated) magnetic field data
)
...
und = SRWLMagFldU(
    [SRWLMagFldH(1, "v", 1.05, 0, 1)], undPer, numPer
)  # Undulator Segment
...
magFldCnt = SRWLMagFldC(
    [und, qf, und, qd, und, qf, und, qd, und], arZero, arZero, arZc
)  # Container of all Field Elements

part.z = (
    arZc[0] - 0.5 * undLenExt
...
xDisp = part.x
yDisp = part.y
dispMagFldCnt = SRWLMagFldC(
    [dispMagFld3D], array("d", [xDisp]), array("d", [yDisp]), array("d", [0])
)
...
partTraj.allocate(
    npTraj, True
)  # True ensures that field along trajectory will also be extracted
...
AuxSaveMagFldData(
    ...
    0,
    0,
    os.path.join(os.getcwd(), strExDataFolderName, strFldOutFileName),
)
AuxSaveTrajData(
