...
harmB.B = 1.0  # magnetic field amplitude [T]
...
stkF.mesh.zStart = 30.0  # longitudinal position [m] at which UR has to be calculated
stkF.mesh.eStart = 10.0  # initial photon energy [eV]
stkF.mesh.eFin = 20000.0  # final photon energy [eV]
...
stkP.mesh.zStart = (
    30.0  # longitudinal position [m] at which power density has to be calculated
)
...
uti_plot1d(
    ...
    ["Photon Energy [eV]", "Flux [ph/s/.1%bw]", "Flux through Finite Aperture"],
)
...
uti_plot2d(
    ...
    plotMeshX,
    plotMeshY,
    ["Horizontal Position [mm]", "Vertical Position [mm]", "Power Density"],
)
...
for i in range(stkP.mesh.ny):
    powDenVsY[i] = stkP.arS[int(stkP.mesh.nx * 0.5) + i * stkP.mesh.ny]
uti_plot1d(
    powDenVsY,
    plotMeshY,
    [
