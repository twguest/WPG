...
strSingleElSpecOutFileName = (
    "ex06_res_single_e_spec.dat"  # file name for output single-e spectrum data
)
strSingleElIntOutFileName = (
    "ex06_res_single_e_int.dat"  # file name for output single-e intensity data
)
...
strSpecIntOutFileName = (
    "ex06_res_spec_int.dat"  # file name for output UR spectral intensity data
)
...
stkF.mesh.zStart = (
    50.0  # 30. #longitudinal position [m] at which UR has to be calculated
)
...
wfrI.mesh.eStart = 7.208e03  # Initial Photon Energy [eV]
...
stkP.mesh.zStart = (
    50.0  # 30. #longitudinal position [m] at which power density has to be calculated
)
...
stkFS = (
    SRWLStokes()
)  # for flux per unit surface vs photon energy, horizontal and vertical position
stkFS.allocate(
    9, 21, 21
)  # numbers of points vs photon energy, horizontal and vertical positions
stkFS.mesh.zStart = 50.0  # longitudinal position [m] at which UR has to be calculated
stkFS.mesh.eStart = 7.14e03  # 10. #initial photon energy [eV]
stkFS.mesh.eFin = 7.26e03  # final photon energy [eV]
...
print(
    "   Performing Single-E Electric Field calculation (vs photon energy) ... ", end=""
)
...
print(
    "   Performing calculation of Spectral Intensity (Stokes parameters) vs photon energy, hor. and vert. positions (takes a bit of time) ... ",
    end="",
)
...
print(
    "   Saving spectral flux [ph/s/.1%bw], flux per unit surface [ph/s/.1%bw/mm^2], poower density [W/mm^2], and electron trajectory data to files ... ",
    end="",
)
...
srwl_uti_save_intens_ascii(
    stkF.arS,
    stkF.mesh,
    os.path.join(os.getcwd(), strExDataFolderName, strFluxOutFileName),
    0,
    ["Photon Energy", "", "", "Flux"],
    _arUnits=["eV", "", "", "ph/s/.1%bw"],
)

AuxSaveTrajData(
    partTraj, os.path.join(os.getcwd(), strExDataFolderName, strTrjOutFileName)
)
srwl_uti_save_intens_ascii(
    arIS,
    wfrS.mesh,
    os.path.join(os.getcwd(), strExDataFolderName, strSingleElSpecOutFileName),
)

srwl_uti_save_intens_ascii(
    arI,
    wfrI.mesh,
    os.path.join(os.getcwd(), strExDataFolderName, strSingleElIntOutFileName),
)

srwl_uti_save_intens_ascii(
    stkP.arS,
    stkP.mesh,
    os.path.join(os.getcwd(), strExDataFolderName, strPowOutFileName),
    0,
...
srwl_uti_save_intens_ascii(
    stkP1.arS,
    stkP1.mesh,
    os.path.join(os.getcwd(), strExDataFolderName, strPowTrjOutFileName),
    0,
    ["", "Horizontal Position", "Vertical Position", "Power Density"],
    _arUnits=["", "m", "m", "W/mm^2"],
)
