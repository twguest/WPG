...
try:
    ...
    print(
        "Can not import scipy for comparison of the numerically calculated intensity distribution with an analytical approximation."
    )
...
strIntOutFileName = (
    "ex16_res_int.dat"  # file name for output SR intensity data before propagation
)
...
srwl.CalcIntFromElecField(
    arIinY, wfrIn, 0, 0, 2, wfrIn.mesh.eStart, 0, 0
)  # extracts intensity
...
for k, driftLength in enumerate([0.0, defaultDriftLength]):
    ...
    srwl.CalcIntFromElecField(
        arII, wfr, polarization, intensity, dependArg, wfr.mesh.eStart, 0, 0
    )
    ...
    srwl.CalcIntFromElecField(
        arI1y, wfr, polarization, intensity, 2, wfr.mesh.eStart, 0, 0
    )  # extracts intensity
    ...
    uti_plot.uti_plot2d1d(
        ...
        labels=[
            "Horizontal Position",
            "Vertical Position",
            "Intenisty at {} m After Aperture".format(driftLength),
        ],
        units=["mm", "mm", "arb. units"],
    )
...
def calc_fwhm(intensities, wavefront, shift=0.5, mesh=True, factor=1e3):
    if mesh:
        ...
        for i in range(wfrIn.mesh.ny):
            y.append(
                (i - wavefront.mesh.ny / 2.0)
                / wavefront.mesh.ny
                * (wavefront.mesh.yFin - wavefront.mesh.yStart)
            )
    else:
        y = wavefront
...
parameters = [
    ...
    [
        "Number of horizontal mesh points before and after propagation",
        wfrIn.mesh.nx,
        wfr.mesh.nx,
    ],
    [
        "Number of vertical mesh points before and after propagation",
        wfrIn.mesh.ny,
        wfr.mesh.ny,
    ],
    [
        "Wavefront horizontal start coordinates [mm] before and after propagation",
        wfrIn.mesh.xStart,
        wfr.mesh.xStart,
    ],
    [
        "Wavefront horizontal end coordinates [mm] before and after propagation",
        wfrIn.mesh.xFin,
        wfr.mesh.xFin,
    ],
    [
        "Wavefront vertical start coordinates [mm] before and after propagation",
        wfrIn.mesh.yStart,
        wfr.mesh.yStart,
    ],
    [
        "Wavefront vertical end coordinates [mm] before and after propagation",
        wfrIn.mesh.yFin,
        wfr.mesh.yFin,
    ],
    [
        "Vertical FWHM [mm] before and after propagation",
        calc_fwhm(arIinY, wfrIn),
        calc_fwhm(arI1y, wfr),
    ],
]
...
if scipy_imported:
    ...
    print(
        "{}Analytical FWHM [mm]: {}".format(
            "    ", calc_fwhm(analyticalIntens, sOut, mesh=False, factor=1.0)
        )
    )
...
try:
    ...
    ax.set_title(
        "Intensity Before and After Propagation\n(cut vs vertical position at x=0)"
    )
    ax.grid()
