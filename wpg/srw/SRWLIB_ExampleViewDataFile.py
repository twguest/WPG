...
if __name__ == "__main__":
    ...
    p.add_option(
        ...
        nargs=0,
        default=0,
        help="read labels from the file header (1) or not (0)",
    )
    p.add_option(
        "-j",
        "--joined",
        ...
        nargs=0,
        default=0,
        help="place different graphs jointly into one figure (1) or into separate figures (0)",
    )
    ...
    p.add_option(
        ...
        action="store_true",
        default=0,
        help="visualize multicolumn data",
    )  # OC14112017 #MR31102017
    p.add_option(
        "--m_colx",
        dest="multicol_data_col_x",
        metavar="STR",
        default=None,
        help="column for horizontal axis",
    )  # OC14112017 #MR31102017
    p.add_option(
        "--m_coly",
        dest="multicol_data_col_y",
        metavar="STR",
        default=None,
        help="column for vertical axis",
    )  # OC14112017 #MR31102017
    p.add_option(
        "-s",
        "--scale",
        ...
        metavar="SCALE",
        default="linear",
        help="scale to use in plots (linear, log, log2, log10)",
    )  # MR20012017
    p.add_option(
        "-w",
        "--width-pixels",
    ...
    if not os.path.isfile(opt.infile):  # MR31102017
        p.error(
            "File name is not specified or is incorrect. Use -f option to specify the file name with path."
        )

    if opt.multicol_data and not (
        opt.multicol_data_col_x and opt.multicol_data_col_y
    ):  # OC14112017 #MR31102017
        p.error(
            "Specify columns for horizontal (--m_colx) and vertical (--m_coly) axes for multicolumn data plot."
        )
    ...
    uti_plot_data_file(
        ...
        opt.joined,
        opt.multicol_data,
        opt.multicol_data_col_x,
        opt.multicol_data_col_y,  # OC14112017 #MR31102017
        opt.scale,
        opt.width_pixels,
    )
