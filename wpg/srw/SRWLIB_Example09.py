...
strMirSurfHeightErrInFileName01 = (
    "mirror1.dat"  # mirror surface height error input file name
)
...
strIntInitOutFileName01 = (
    "ex09_res_int_in.dat"  # initial wavefront intensity distribution output file name
)
strPhaseInitOutFileName01 = (
    "ex09_res_phase_in.dat"  # initial wavefront phase output file name
)
strIntPropOutFileName01 = "ex09_res_int_prop.dat"  # propagated wavefront intensity distribution output file name
strPhasePropOutFileName01 = (
    "ex09_res_phase_prop.dat"  # propagated wavefront phase output file name
)
...
print(
    "RMS Source Size:",
    round(GsnBm.sigX * 1.0e06, 3),
    "micro-m; RMS Divergence:",
    round(rmsAngDiv * 1.0e06, 3),
    "micro-rad",
)
...
firstHorAp = 8.0 * rmsAngDiv * distSrc_M1  # [m]
...
# Aperture of M1
opApM1 = SRWLOptA("r", "a", 10.0e-03, lenM1 * angM1)

# M1 Surface Height Error
# Read mirror slope arror data from file and setup the corresponding optical element
print(
    "Defining Transmission element (to simulate mirror surface slope error)...", end=""
)
heightProfData = srwl_uti_read_data_cols(
    os.path.join(os.getcwd(), strDataFolderName, strMirSurfHeightErrInFileName01),
    _str_sep="\t",
    _i_col_start=0,
    _i_col_end=1,
)
...
srwl_uti_save_intens_ascii(
    opPathDifErM1,
    opTrErM1.mesh,
    os.path.join(os.getcwd(), strDataFolderName, strMirOptPathDifOutFileName01),
    0,
...
opTrErVFM = srwl_opt_setup_surf_height_1d(
    heightProfData, _dim="y", _ang=angKB, _amp_coef=1
)
# Use _amp_coef != 1 to scale surface height error
print("done")
print("Saving optical path difference data to file (for viewing/debugging)...", end="")
...
srwl_uti_save_intens_ascii(
    opPathDifErVFM,
    opTrErVFM.mesh,
    os.path.join(os.getcwd(), strDataFolderName, strMirOptPathDifOutFileName02),
    0,
    ["", "Horizontal Position", "Vertical Position", "Opt. Path Diff."],
    _arUnits=["", "m", "m", "m"],
)
...
# VFM simulated by Extended Elliptical Mirror:
opHFM = SRWLOptMirEl(
    _p=(distSrc_M1 + distM1_VFM + distVFM_HFM),
    _q=distHFM_Samp,
    _ang_graz=angKB,
    _size_tang=lenKB,
    _size_sag=10.0e-03,
...
opTrErHFM = srwl_opt_setup_surf_height_1d(
    heightProfData, _dim="x", _ang=angKB, _amp_coef=1
)
# Use _amp_coef != 1 to scale surface height error
print("done")
print("Saving optical path difference data to file (for viewing/debugging)...", end="")
...
srwl_uti_save_intens_ascii(
    opPathDifErHFM,
    opTrErHFM.mesh,
    os.path.join(os.getcwd(), strDataFolderName, strMirOptPathDifOutFileName03),
    0,
    ["", "Horizontal Position", "Vertical Position", "Opt. Path Diff."],
    _arUnits=["", "m", "m", "m"],
)
...
prParInit = [0, 0, 1.0, 1, 0, 2.0, 5.0, 6.0, 3.0, 0, 0, 0]
...
prParPost = [
    ...
    0,
    0,
]  # Post-propagation resizing parameters
...
srwl_uti_save_intens_ascii(
    arI0,
    mesh0,
    os.path.join(os.getcwd(), strDataFolderName, strIntInitOutFileName01),
    0,
    ["Photon Energy", "Horizontal Position", "Vertical Position", "Spectral Fluence"],
    _arUnits=["eV", "m", "m", "J/eV/mm^2"],
)
...
arP0 = array(
    "d", [0] * mesh0.nx * mesh0.ny
)  # "flat" array to take 2D phase data (note it should be 'd')
srwl.CalcIntFromElecField(
    arP0, wfr, 6, 4, 3, mesh0.eStart, 0, 0
)  # extracts radiation phase
srwl_uti_save_intens_ascii(
    arP0,
    mesh0,
    os.path.join(os.getcwd(), strDataFolderName, strPhaseInitOutFileName01),
    0,
    ["Photon Energy", "Horizontal Position", "Vertical Position", "Phase"],
    _arUnits=["eV", "m", "m", "rad"],
)
...
srwl_uti_save_intens_ascii(
    arI1,
    mesh1,
    os.path.join(os.getcwd(), strDataFolderName, strIntPropOutFileName01),
    0,
    ["Photon Energy", "Horizontal Position", "Vertical Position", "Spectral Fluence"],
    _arUnits=["eV", "m", "m", "J/eV/mm^2"],
)
...
srwl.CalcIntFromElecField(
    arP1, wfr, 6, 4, 3, mesh1.eStart, 0, 0
)  # extracts radiation phase
srwl_uti_save_intens_ascii(
    arP1,
    mesh1,
    os.path.join(os.getcwd(), strDataFolderName, strPhasePropOutFileName01),
    0,
    ["Photon Energy", "Horizontal Position", "Vertical Position", "Phase"],
    _arUnits=["eV", "m", "m", "rad"],
)
...
uti_plot2d(
    ...
    [
        ...
        "Spectral Fluence Before Propagation",
    ],
)
uti_plot1d(
    arI0x,
    ...
    [
        ...
        "Spectral Fluence (horizontal cut at y = 0)",
    ],
)
uti_plot1d(
    arI0y,
    ...
    [
        "Vertical Position [mm]",
        "Spectral Fluence [J/eV/mm^2]",
        "Spectral Fluence (vertical cut at x = 0)",
    ],
)
# uti_plot2d(arP0, plotMesh0x, plotMesh0y, ['Horizontal Position [mm]', 'Vertical Position [mm]', 'Phase Before Propagation'])

plotMirErrMesh0x = [
    1000 * opTrErVFM.mesh.xStart,
    1000 * opTrErVFM.mesh.xFin,
    opTrErVFM.mesh.nx,
]
plotMirErrMesh0y = [
    1000 * opTrErVFM.mesh.yStart,
    1000 * opTrErVFM.mesh.yFin,
    opTrErVFM.mesh.ny,
]
uti_plot2d(
    opPathDifErVFM,
    ...
    [
        "Horizontal Position [mm]",
        "Vertical Position [mm]",
        "Optical Path Difference due to VFM Error",
    ],
)
uti_plot1d(
    opPathDifErVFMy,
    plotMirErrMesh0y,
    ["Vertical Position [mm]", "Opt. Path. Diff. [m]", "Optical Path Difference"],
)
...
uti_plot2d(
    ...
    [
        ...
        "Spectral Fluence After Propagation",
    ],
)
uti_plot1d(
    arI1x,
