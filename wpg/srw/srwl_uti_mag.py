...
# ****************************************************************************
def srwl_mag_kick(_el_en=3.0, _ang=1.0, _x_or_y="x", _len=1.0, _led=0):
    ...
    if _len <= 0:
        raise Exception(
            "Inconsistent input magnet parameters: effective length can not be negative"
        )
    ...
    if _led > 0.0:
        ...
        if L0 < 0.0:
            raise Exception(
                "Inconsistent input magnet parameters: magnet edge length if too large for given effective length"
            )
...
def srwl_mag_extrap_grad_off_mid_plane(_mag_mid, _ry, _ny, _grad_mult=1):
    ...
    if (
        (_mag_mid is None)
        or (_mag_mid.arMagFld is None)
        or (len(_mag_mid.arMagFld) != 1)
        or (_ry < 0.0)
        or (_ny <= 1)
    ):
        raise Exception(
            "Incorrect input parameters for magnetic field to be extrapolated"
        )
    ...
    fld3dRes = SRWLMagFld3D(
        arBxRes, arByRes, arBzRes, nx, ny, nz, fld3d_mid.rx, _ry, fld3d_mid.rz
    )
...
# ****************************************************************************
def srwl_mag_track_e_beam_mom(
    _e_beam, _mag, _z_or_ct, _ct_start=0, _ct_end=0, _npart=1000, _np_in_trj=10000
):
    ...
    if _ct_start != _ct_end:  # treat _z_or_ct as ct
        ...
        if treatCT is not True:
            raise Exception(
                "Trajectory point corresponding to given time moment was not found"
            )
        ...
        if treatCT is False:
            ...
            while iz >= 0:
                if iz < np_in_trj_mi_1:
                    if (partTraj.arZ[iz] <= _z_or_ct) and (
                        _z_or_ct < partTraj.arZ[iz + 1]
                    ):
                        ind0 = iz
                        newIndFound = True
                        break
            ...
            if newIndFound is False:
                raise Exception(
                    "Trajectory point corresponding to given longitudinal position was not found"
                )

            r0 = (_z_or_ct - partTraj.arZ[ind0]) / (
                partTraj.arZ[ind0 + 1] - partTraj.arZ[ind0]
            )
