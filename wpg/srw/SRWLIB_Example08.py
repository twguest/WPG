...
print(
    "Simulating emission and propagation of undulator radiation (UR) wavefront through a simple optical scheme including CRL"
)
print("")
print(
    "First, single-electron UR (on-axis spectrum and a wavefront at a fixed photon energy) is calculated and propagated through the optical scheme. ",
    end="",
)
print(
    'After this, calculation of partially-coherent UR from entire electron beam is started as a loop over "macro-electrons", using "srwl_wfr_emit_prop_multi_e" function. ',
    end="",
)
print(
...
strIntOutFileNamePartCoh = (
    "ex08_res_int_part_coh1.dat"  # file name for output SR intensity data
)
strOpTrFileName = (
    "ex08_res_op_tr_crl.dat"  # file name for output optical transmission data
)
strOpPathDifFileName = (
    "ex08_res_op_path_dif_crl.dat"  # file name for optical path difference data
)
...
und = SRWLMagFldU(
    [SRWLMagFldH(1, "v", By, phBy, sBy, 1), SRWLMagFldH(1, "h", Bx, phBx, sBx, 1)],
    undPer,
    numPer,
...
elecBeam.partStatMom1.x = 0.0  # Initial Transverse Coordinates (initial Longitudinal Coordinate will be defined later on) [m]
elecBeam.partStatMom1.y = 0.0
elecBeam.partStatMom1.z = (
    0.0  # -0.5*undPer*(numPer + 4) #Initial Longitudinal Coordinate (set before the ID)
)
elecBeam.partStatMom1.xp = 0  # Initial Relative Transverse Velocities
elecBeam.partStatMom1.yp = 0
elecBeam.partStatMom1.gamma = 7.0 / 0.51099890221e-03  # Relative Energy
...
wfr1.mesh.eStart = 1000.0  # Initial Photon Energy [eV]
wfr1.mesh.eFin = 10000.0  # Final Photon Energy [eV]
wfr1.mesh.xStart = 0.0  # Initial Horizontal Position [m]
...
fx = 1e23  # Focal Length in Horizontal plane
...
geomApertNF = (
    3e-03  # [m] Geometrical aparture of 1D CRL in the plane where there is no focusing
)
rMin = 0.5e-03  # [m] radius at tip of parabola of CRL
nCRL = 3
wallThick = 50e-06  # [m] wall thickness of CRL

optCRL = srwl_opt_setup_CRL(
    2, delta, attenLen, 1, geomApertNF, geomApertF, rMin, nCRL, wallThick, 0, 0
)  # 1D CRL
...
srwl_uti_save_intens_ascii(
    optTrIntCRL,
    optCRL.mesh,
    os.path.join(os.getcwd(), strExDataFolderName, strOpTrFileName),
    0,
    ["", "Horizontal Position", "Vertical Position", "Intensity Transmission"],
    _arUnits=["", "m", "m", "r.u."],
)
...
srwl_uti_save_intens_ascii(
    optPathDifCRL,
    optCRL.mesh,
    os.path.join(os.getcwd(), strExDataFolderName, strOpPathDifFileName),
    0,
...
propagParApert = [0, 0, 1.0, 0, 0, 1.5, 1.0, 1.1, 6.0, 0, 0, 0]
propagParLens = [0, 0, 1.0, 0, 0, 1.0, 1.0, 1.0, 1.0, 0, 0, 0]
propagParDrift = [0, 0, 1.0, 1, 0, 1.0, 1.2, 1.0, 1.0, 0, 0, 0]
...
optBL = SRWLOptC(
    [optApert, optCRL, optDrift], [propagParApert, propagParLens, propagParDrift]
...
if srwl_uti_proc_is_master():
    ...
    srwl.CalcIntFromElecField(
        arI1, wfr1, 6, 0, 0, wfr1.mesh.eStart, wfr1.mesh.xStart, wfr1.mesh.yStart
    )
    print("done")
    ...
    srwl_uti_save_intens_ascii(
        arI1,
        wfr1.mesh,
        os.path.join(os.getcwd(), strExDataFolderName, strIntOutFileName1),
        0,
    )
    print("done")
    ...
    arPrecPar[
        6
    ] = sampFactNxNyForProp  # sampling factor for adjusting nx, ny (effective if > 0)
    srwl.CalcElecFieldSR(wfr2, 0, magFldCnt, arPrecPar)
    print("done")
    print(
        "Extracting Intensity from the Calculated Initial Electric Field ... ", end=""
    )
    arI2 = array(
        "f", [0] * wfr2.mesh.nx * wfr2.mesh.ny
    ...
    srwl_uti_save_intens_ascii(
        arI2,
        wfr2.mesh,
        os.path.join(os.getcwd(), strExDataFolderName, strIntOutFileName2),
        0,
    )
    print("done")
    ...
    srwl_uti_save_intens_ascii(
        arI3,
        wfr2.mesh,
        os.path.join(os.getcwd(), strExDataFolderName, strIntOutFileName3),
        0,
    )
    print("done")
