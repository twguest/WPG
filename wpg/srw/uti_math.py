...
def interp_1d(_x, _x_min, _x_step, _nx, _ar_f, _ord=3, _ix_per=1, _ix_ofst=0):
    ...
    """
    if _ord == 1:
        i0 = int(trunc((_x - _x_min) / _x_step + 1.0e-09))
        if i0 < 0:
            i0 = 0
        elif i0 >= _nx - 1:
            i0 = _nx - 2
        ...
        a1 = -0.5 * a0 + f1 - f2 / 6.0 - fm1 / 3.0
        a2 = -a0 + 0.5 * (f1 + fm1)
        a3 = 0.5 * (a0 - f1) + (f2 - fm1) / 6.0
...
def interp_1d_var(_x, _ar_x, _ar_f, _ord=3):
    ...
    sErrBadArrays = (
        "Incorrect/incompatible lengths of argument and function value arrays."
    )

    nx = len(_ar_x)
    if nx <= 0:
        raise Exception(sErrBadArrays)
    nf = len(_ar_f)
    if nf <= 0:
        raise Exception(sErrBadArrays)
    ...
    if _x <= _ar_x[0]:
        return _ar_f[0]
    elif _x >= _ar_x[nx_mi_1]:
        return _ar_f[nx_mi_1]
    ...
    if _ord == 1:
        ...
        invD = 1.0 / (
            dxm1 * dxp1 * dxp2 * (dxm1 - dxp1) * (dxm1 - dxp2) * (dxp1 - dxp2)
        )
        invD1 = 1.0 / (dxm1 * dxp1 * dxp2)
        dxm1e2 = dxm1 * dxm1
        dxm1e3 = dxm1e2 * dxm1
        dxp1e2 = dxp1 * dxp1
        dxp1e3 = dxp1e2 * dxp1
        ...
        dxp2e3 = dxp2e2 * dxp2
        a1 = (
            -dxp1e2 * (dxp1 - dxp2) * dxp2e2 * (f0 - fm1)
            + dxm1e2 * (dxp2e3 * (fp1 - f0) + dxp1e3 * (f0 - fp2))
            + dxm1e3 * (dxp2e2 * (f0 - fp1) + dxp1e2 * (fp2 - f0))
        ) * invD
        a2 = ((dxm1 + dxp1 + dxp2) * f0) * invD1 + (
            dxm1 * dxp2 * (dxm1e2 - dxp2e2) * fp1
            + dxp1e3 * (dxm1 * fp2 - dxp2 * fm1)
            + dxp1 * (dxp2e3 * fm1 - dxm1e3 * fp2)
        ) * invD
        a3 = (
            -f0 * invD1
            + (
                dxm1 * dxp2 * (dxp2 - dxm1) * fp1
                + dxp1e2 * (dxp2 * fm1 - dxm1 * fp2)
                + dxp1 * (dxm1e2 * fp2 - dxp2e2 * fm1)
            )
            * invD
        )
        dx = _x - x0
...
def interp_2d(
    ...
    if _ord == 1:  # bi-linear interpolation based on 4 points
        ix0 = int(trunc((_x - _x_min) / _x_step + 1.0e-09))
        if ix0 < 0:
            ix0 = 0
        elif ix0 >= _nx - 1:
            ix0 = _nx - 2
        ...
        iy0 = int(trunc((_y - _y_min) / _y_step + 1.0e-09))
        if iy0 < 0:
            iy0 = 0
        elif iy0 >= _ny - 1:
            iy0 = _ny - 2
        ...
        return (
            a00
            + tx * (a10 + tx * (a20 + tx * (a30 + ty * a31) + ty * a21) + ty * a11)
            + ty * (a01 + ty * (a02 + ty * (a03 + tx * a13) + tx * a12))
        )
    return 0
...
def find_ar_max(_ar, _ib=0, _ie=-1, _min=False):
    ...
    if _ar is None:
        raise Exception(strErIncorArray)
    nTot = len(_ar)
    if nTot <= 0:
        raise Exception(strErIncorArray)
    ...
    iEnd = _ie
    if (iEnd == -1) or (_ie >= nTot):
        iEnd = nTot - 1

    if iEnd < iBeg:
        raise Exception("Incorrect definition of start and end indexes.")
...
def integ_ar_2d(_ar, _ar_align, _x_grid, _y_grid, _x_lim=None, _y_lim=None):
    ...
    if _y_lim is not None:
        ...
        y_max = _y_lim[1]
        if len(_y_lim) > 2:
            nyInteg = int(round(_y_lim[2]))

    if (
        (nxInteg > 2) and (nyInteg > 2) and (_ar_align == 1)
    ):  # perform inregration using 2D interpolation
        ...
        for iy in range(nyInteg):
            ...
            for ix in range(nxInteg):
                arAuxIntegWave2Dx[ix] = interp_2d(
                    xx, yy, xStart, xStep, nx, yStart, yStep, ny, _ar, _ord=2
                )
                xx += xStepInteg
    ...
    ixStart = int((x_min - xStart) / xStep + 0.0e-12)
    if ixStart < 0:
        ixStart = 0
    else:
        if ixStart >= nx:
    ...
    iyStart = int((y_min - yStart) / yStep + 0.0e-12)
    if iyStart < 0:
        iyStart = 0
    else:
        if iyStart >= ny:
    ...
    ixEnd = int((x_max - xStart) / xStep - 1.0e-06) + 1
    if ixEnd < 0:
        ixEnd = 0
    else:
        if ixEnd >= nx:
    ...
    iyEnd = int((y_max - yStart) / yStep - 1.0e-06) + 1
    if iyEnd < 0:
        iyEnd = 0
    else:
        if iyEnd >= ny:
    ...
    if (ixStart >= ixEnd) or (iyStart >= iyEnd):
        return 0.0
    ...
    for iy in range(nyi):
        ...
        for ix in range(nxi):
            ixAbs = ix + ixStart
            if _ar_align == 1:
                arAuxIntegWave2Dx[ix] = _ar[iyAbs_nx + ixAbs]
            else:
                arAuxIntegWave2Dx[ix] = ar_iyAbs[ixAbs]
...
def matr_prod(_A, _B):
    ...
    if (
        isinstance(B0, list) or isinstance(B0, array) or isinstance(B0, tuple)
    ):  # _B is matrix
        lenB0 = len(B0)
...
def matr_3x3_inv(_M):
    ...
    S0i = [
        invDet * (S1[1] * S2[2] - S1[2] * S2[1]),
        invDet * (-S0[1] * S2[2] + S0[2] * S2[1]),
        invDet * (S0[1] * S1[2] - S0[2] * S1[1]),
    ]
    S1i = [
        invDet * (-S1[0] * S2[2] + S1[2] * S2[0]),
        invDet * (S0[0] * S2[2] - S0[2] * S2[0]),
        invDet * (-S0[0] * S1[2] + S0[2] * S1[0]),
    ]
    S2i = [
        invDet * (S1[0] * S2[1] - S1[1] * S2[0]),
        invDet * (-S0[0] * S2[1] + S0[1] * S2[0]),
        invDet * (S0[0] * S1[1] - S0[1] * S1[0]),
    ]
    return [S0i, S1i, S2i]
...
def trf_rotation(_V, _ang, _P):
    ...
    """
    normFact = 1.0 / sqrt(_V[0] * _V[0] + _V[1] * _V[1] + _V[2] * _V[2])
    ...
    M = [st0, st1, st2]
    st00 = [1.0 - st0[0], -st0[1], -st0[2]]
    st01 = [-st1[0], 1.0 - st1[1], -st1[2]]
    st02 = [-st2[0], -st2[0], 1.0 - st2[2]]
    M0 = [st00, st01, st02]
    V = matr_prod(M0, _P)
    return [M, V]
...
def fwhm(x, y, shift=0.5, return_as_dict=False):  # MR21032017
    ...
    maxY = max(y)
    if maxY == minY:
        raise Exception("FWHM can not be calculated")
    mult = 1.0 / (maxY - minY)
    lenY = len(y)
    for i in range(lenY):
        y[i] = (y[i] - minY) * mult - shift
    ...
    for i in range(lenY):
        ...
        if current_positive != positive:
            list_of_roots.append(
                x[i - 1]
                + (x[i] - x[i - 1]) / (abs(y[i]) + abs(y[i - 1])) * abs(y[i - 1])
            )
