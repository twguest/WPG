...
def _multicolumn_file_load(fname):  # MR31102017
    ...
    data = uti_io.read_ascii_data_cols(
        fname, "\t", _i_col_start=0, _i_col_end=-1, _n_line_skip=1
    )
    d = {}
...
def _traj_file_load(fname, traj_axis="x"):  # MR20160725
    ...
    data = uti_io.read_ascii_data_cols(
        fname, "\t", _i_col_start=0, _i_col_end=10, _n_line_skip=nLinesHead
    )
    ...
    if traj_axis == "x":
        ...
        raise ValueError(
            'Parameter "axis" has wrong value: {}. Allowed values are "x" and "y"'.format(
                traj_axis
            )
        )
...
def _file_load(_fname, _read_labels=1):  # MR20160725
    ...
    data = uti_io.read_ascii_data_cols(
        _fname, "\t", _i_col_start=0, _i_col_end=0, _n_line_skip=nLinesHead
    )[0]
...
def rescale_range(allrange, _ar_units, _ec=0, _xc=0, _yc=0):
    ...
    e0s = uti_math.num_round(
        e0 * mult_e
    )  # this is done to avoid in the labels numbers like "12.700000000001"
