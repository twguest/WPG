...
def srwl_wfr_emit_prop_multi_e(
    output, _e_beam, _mag, _mesh, ne
):  # OC07092016 (added _wr)
    """
    ...
    meshRes = SRWLRadMesh(
        ...
        _mesh.ny,
        _mesh.zStart,
    )  # to ensure correct final mesh if _opt_bl==None
...
if __name__ == "__main__":
    ...
    output = np.memmap(
        r"./extensions/out/tmp/memmap_me",
        dtype=wfr.data.arrEhor.dtype,
        shape=wfr.data.arrEhor.shape,
        mode="w+",
    )
    ...
    beamline.setup_OE()
    _mesh = Mesh(
        _eStart=beamline.params.w_e,
        _eFin=beamline.params.w_e,
        _ne=1,
        _nx=2048,
        _ny=2048,
        _xStart=-20e-04,
    ...
    beamline.setup_src()
    srwl_wfr_emit_prop_multi_e(output, beamline.elecBeam, beamline.magFldCnt, _mesh, 1)
