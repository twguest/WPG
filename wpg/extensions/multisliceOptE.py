...
from extensions.gvrutils import propagationParameters, plotWavefront,plot_wfront2
from extensions.gvrutils import writeIntensity, pixelScale, get_transmissionFunctionCplx,  get_intensity_on_axis
from extensions.gvrutils import show_transmissionFunctionCplx
...
class SRWLUtiSmp:
    ...
    def __init__(self, inputData,
                 ...
                 tile=None, shift_x=None, shift_y=None, invert=None, invert_bitwise=None,
                 is_show_images=True, is_save_images=True,
        ...
        output_image_format = os.path.splitext(inputData)[1].replace('.', '') if not output_image_format else \
            output_image_format
    ...
    def preprocess_input(self ):
        ...
        if self.shift_x:
            ...
            if self.shift_x > 0:
                self.data = np.pad(self.data, ((0, 0), (self.shift_x, 0)), mode='edge')[:, :-self.shift_x]
                ...
                self.data = np.pad(self.data, ((0, 0), (0, -self.shift_x)), mode='edge')[:, -self.shift_x:]
    ...
    def pad(self):
           assert type(self.padding) in [tuple, list] and len(self.padding) == 4, \
                'The pad parameters should be a list/tuple and contain 4 elements (top,bottom,left, right,)'
           assert self.padding[0] >=0 and self.padding[1] >=0 and self.padding[2] >=0 and self.padding[3] >=0, \
    ...
    def _check_files(self):
        if not os.path.isfile(self.inputData):
            raise ValueError('Provided file "{}" does not exist.'.format(self.inputData))
...
def srwl_opt_setup_transm_from_file(
        ...
        is_save_images=False, prefix='', output_image_format=None, pad=None
):
...
class greyscaleToSlices():
    ...
    def __init__(self, inputData, 
                 ...
                 slices = 1,
                 area=None, 
                 rotate_angle=0, rotate_reshape=True, 
                 cutoff_background_noise=0, background_color=0,
                 tile=None, shift_x=None, shift_y=None, invert=None, invert_bitwise=None,
    ...
    def multiSliceTr(self):
        ...
        material=[(self.thickness/self.greyLevels/self.greyLevels), self.delta, self.attenLength]
    ...
    def slicer(self):
        ...
        greyLevelUpper = np.round(np.linspace(self.limit_value,0,self.greyLevels+1)[0:self.greyLevels])
        greyLevelLower = np.round(np.linspace(self.limit_value,0,self.greyLevels+1)[1:self.greyLevels+1])
        ...
        if parproc:
            ...
            with pymp.Parallel(NumProcesses) as p:
                for index in p.range(0,len(list(greyLevelRanges))-1):
                     ...
                     gs[(gs>rl) & (gs<=rh)] = rl   # object minus slice, i.e. what is left of the cake
                     s = s - gs    # the slice
                     img = Image.fromarray(s.astype(np.uint8))
                     outPath = os.path.join(self.save_dir, self.processed_image_name + str(index) + '.tif')
                     img.save(outPath)
            ...
            for rl,rh,i in greyLevelRanges:
                 ...
                 print('Slicing, slice number %d - greyvalues in interval[%d,%d]' % (i, rl, rh))
                 s = np.where(gs>rl, gs-rl, 0)   # the slice
                 print('rl is {}'.format(rl))
                 #s = np.subtract(s, rl)
                 if i == ( len(greyLevelRanges)-1): # last slice
                       s = np.subtract(s,np.min(s[s!=0]-2))  #correction, but why +2??, i.e. why do negative values appear in last slice
                       s[s==255]=0
                 
                 gs = np.where((gs>rl),rl, gs) # the residual           

                 print ('Greyscale values in slice in interval [%d, %d]' % (np.min(s), np.max(s)))
                 print ('Greyscale values in residual object in interval [%d, %d]' % (np.min(gs), np.max(gs)))
                 ...
                 imSlice = axSlice.imshow(s,cmap='gray')#, vmin=0, vmax=self.limit_value)
                 fig.canvas.draw()
    
                 img = Image.fromarray(s.astype(np.uint8))
                 outPath = os.path.join(self.save_dir, self.processed_image_name + str(i) + '.tif')
                 img.save(outPath)
    ...
    def pad(self):
           ...
           self.data = np.padding(self.data, ((self.padding[0],self.padding[1]), (self.padding[2], self.padding[3])), 'constant', constant_values =self.background_color)
...
def testblObjectSliced(N,trFilePath,orientation='x',
...
of the object the x or y axis.
    ...
    if trPreFilePath is not None:
        ...
        slices.append ([trPreFilePath,[0.,0.,0.,0.],resolution, trP ,None,pp] )
...
#        return thickness  
    ...
    def getAttenuationLength(self):
        ...
        '''
        val =  -0.5*np.divide(self.getThickness(),math.log(getAmplitudeTransmission()))
    ...
    def getPhaseShift(self, wavelength):
        ...
        '''
        val =  2*math.pi*np.multiply(self.getDelta(),self.getThickness()) / wavelength
    ...
    def addToSumTrans(self, tr):
       ...
       # this is old and wrong!!  don't us it
        ...
        if len(tr.arTr) == len(self.sumTrans.arTr):
                ...
                print('Warning - refusing to add transmission funnctions of different dimensions')
    ...
    def addToSumThickness(self, tr):                  
        if self.sumThickness is None:
            ...
            if len(tr.pathInBody) == len(self.sumThickness):
                ...
                print('Warning - cowardly refusing to add thickness of slice with different number of elements')
    ...
    def generateSlicesFromImage(self,
...
# =============================================================================
        ...
        for i in range(N):
            ...
            slices.append( [imgF[i],translation[i],resolution[i],material[i],pad[i],propagationParameters()] )
...
#        return self.getSlice(self.sliceIndex)
    ...
    def printSliceParams(self):
        ...
        table.column_headers = ["Image Path", "Translation", "Resolution", "Material", "Pad","PropagationParameters"]
    ...
    def getSlice(self,n,show=False, outFilePath = None, saveProcessedImage=False, driftSpaces=True):
...
#              imgPath0=''
#          
          imgPath,trans,resolution,material,pad,propagationParams = self.getSliceParam(n)
       ...
       #   propagationDistance = z-z0
          ...
          el.append(tr)
          pp.append(propagationParameters()) # use simplest assumption for drift space propagation paramters.  To be improved!
          ...
          if outFilePath:
                incremental_filename = os.path.join(outFilePath, "%04d_%s.tr" % (n,'slice'))
    ...
    def propagateSlices(self,wfr,outFilePath=None,showSliceSummary=False,plot=False):
        """
...
def testgreyscaleToSlices(N):
    ...
    msobject= greyscaleToSlices('/opt/wpg/wpg/extensions/in/ic.tif',
                             slices=N,
                             invert=False,  
    ...
    plotWavefront(wf, 'Wavefield propagated through multislice object', )
    writeIntensity(wf,'AfterObject',path='./out',polarization='horizontal',imgType='tif')
...
def testgreyscaleToSlicesForLTZP():
    ...
    wf=constructTestWaveField(z1=15)# alaleh's choice of z1, but moved to argument here, change to function undone)
    ...
    msobject= greyscaleToSlices(thicknessMap,
                                Use_PP(semi_analytical_treatment=1.0,zoom=0.3, sampling=1),  # should reconsider zoom and sampling here
                                slices=N,
                                invert=False,  
    ...
    print('I0={}, I1={}, Absorption={}, Transmission={}'.format(I0_total, I1_total, absorption, transmission))
...
def test_multislice_from_images():
    ...
    # now save tr
    multiSliceLTZP_X.writeTransmissionFunction('out.pickle', 
