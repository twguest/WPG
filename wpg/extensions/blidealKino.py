...
def testKINOFORMBeamline(conf,outPath,roughHeight):
# =============================================================================
...
#                                  writePhase=True)
  ...
  #  blfocusVol.description=p.focusVol.description 
    ...
    if not( os.path.exists(p.CACHEPATH+p.wfPostZPXPath) or (os.path.exists(p.CACHEPATH+p.wfFocusXPath))):    
        ...
        blCleanupOptic = bl(SRWLOptC(_arOpt=[SRWLOptA("r", "a", p.KINOFORMx.aperture.xwidth, p.KINOFORMx.aperture.ywidth, 0.0, 0.0)],
        ...
        p.KINOFORMx.slices = testblObjectSliced(p.KINOFORMx.NSlices,
                                          ...
                                          thickness   = p.KINOFORMx.thickness,
                                          delta       = p.KINOFORMx.delta,
                                          attenLength = p.KINOFORMx.attenLength,
                                          trPreFilePath = p.KINOFORMx.trPreFile,
                                          trPreProperties = p.KINOFORMx.trPreProperties
                                         )
...
#                         blSaveDetector]
    ...
    #drift region between optics
       ...
       # blDriftXY.description = 'Drift KINOFORMX to KINOFORMY'
    ...
    #                                    )
        ...
        #multiSliceKINOFORM_Y = multisliceOptE(objectSlices=slices,description='KINOFORM (multislice)')
        #multiSliceKINOFORM_Y.printSliceParams()
        #multiSliceKINOFORM_X.description = 'KINOFORM Y'
...
#             
    ...
    if multiSliceKINOFORM_X in OpticalSystem: 
    ...
    # now save tr
    multiSliceKINOFORM_X.writeTransmissionFunction(strOutPath+'/kino_transmission.pickle', 
