...
def make_cube_stamp(cube_nx, axis=[1, 1, 1], angle=np.pi / 6, save=False):
    ...
    qs = Quaternion(axis=axis, angle=angle)  # make an object that defines the rotation of the points

    # Get the coordinates in the cube
    # cube_points = [ [0,0,0], [0,0,1], ..., [nx, nx, nx] ]
    cube_points = np.argwhere(cube == 1)

    rotated_cube = np.dot(cube_points, qs.rotation_matrix)  # rotate the points to find the rotated cube

    rotated_cube = np.round(rotated_cube - np.min(rotated_cube)).astype(int)  # translate the points so everything is +
...
def make_n_slices(sample, n=10, save=False, fname='slice_chunk'):
    ...
    for i in range(n):
        ...
        if save:
            imageio.imwrite('sample_slices\\' + fname + str(i) + '.tiff', chunk_2d.astype(int))
...
def stamp_cube_into_sample(cube, sample):
    ...
    new_sample[stamp_loc_xs:stamp_loc_xs + cube.shape[0], \
