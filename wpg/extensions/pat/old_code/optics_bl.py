...
total_beamline = beamline.Beamline()  # Init an empty beamline for all the optical elements

for i in range(len(op.arOpt)):  # loop through each optical element (-2 to chop outlast two elements)
...
if linux:
    ...
    imageio.imwrite('io/optics_bl/log/' + 'log10_' + str(0) + '.tiff', np.log10(wf_wpg_inten))
else:
...
for i in range(len(bls)):
    ...
    if linux:
        ...
        imageio.imwrite('io/optics_bl/log/' + 'log10_' + str(i + 1) + '.tiff', log10_inten)
    else:
