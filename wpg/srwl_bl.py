...
class SRWLBeamline(object):
    ...
    def set_e_beam(
        ...
        if len(_e_beam_name) > 0:
            self.eBeam = srwl_uti_src_e_beam(
                _e_beam_name, _sig_e=_ens, _emit_x=_emx, _emit_y=_emy
            )
            ...
            if self.eBeam is None:
                if (_e_beam is None) or (isinstance(_e_beam, SRWLPartBeam) is False):
                    ...
                    raise ValueError(
                        'The beam name "{}" was not found in the database and _e_beam is empty'.format(
                            _e_beam_name
                        )
                    )
                else:
        ...
        if (eBeamWasSetFromDB is False) and (
            eBeamWasSetFromInObj is False
        ):  # Try to set up e-beam from input Twiss params or Moments
            ...
            if (_alphax is None) or (_alphay is None):
                twiss_check = False  # OC28092016
            if (_etaxp is None) or (_etayp is None):
                twiss_check = False

            moments_check, moments_bad_var = check_positive(moments_inputs)
            if (_mxxp is None) or (_myyp is None):
                moments_check = False  # OC28092016
            ...
            if twiss_check:
                ...
                raise ValueError(
                    err_msg.format(
                        help_dict["ebm{}".format(twiss_bad_var)],
                        twiss_bad_var,
                        twiss_inputs[twiss_bad_var],
                        help_dict["ebm{}".format(moments_bad_var)],
                        moments_bad_var,
                        moments_inputs[moments_bad_var],
                    )
                )
            # else:
            ...
            if (_ens is not None) and (_ens > 0):
                self.eBeam.arStatMom2[10] = _ens * _ens
            if (_sigx is not None) and (_sigx > 0):
                self.eBeam.arStatMom2[0] = _sigx * _sigx
            if (_sigxp is not None) and (_sigxp > 0):
                self.eBeam.arStatMom2[2] = _sigxp * _sigxp
            if (_mxxp is not None) and (_mxxp != 1.0e23):
                self.eBeam.arStatMom2[1] = _mxxp
            if (_sigy is not None) and (_sigy > 0):
                self.eBeam.arStatMom2[3] = _sigy * _sigy
            if (_sigyp is not None) and (_sigyp > 0):
                self.eBeam.arStatMom2[5] = _sigyp * _sigyp
            if (_myyp is not None) and (_myyp != 1.0e23):
                self.eBeam.arStatMom2[4] = _myyp
        ...
        if (_dr is not None) and (_dr != 0):
            self.eBeam.drift(_dr)  # OC16032017 (see above)

        if (_de is not None) and (_de != 0):
            elRestMassGeV = 0.51099890221e-03
            curE0 = (
                self.eBeam.partStatMom1.gamma
                * self.eBeam.partStatMom1.relE0
                * elRestMassGeV
            )
            self.eBeam.partStatMom1.gamma = (curE0 + _de) / (
                self.eBeam.partStatMom1.relE0 * elRestMassGeV
            )
    ...
    # ------------------------------------------------------------------------
    def set_mag_multipole(
        self, _bx=0, _by=1.0, _gn=0, _gs=0, _len=1.5, _led=0, _r=0, _zc=0, _add=0
    ):
        ...
        if self.mag_approx is None:
            self.mag_approx = SRWLMagFldC()

        if _bx != 0:
            dipBx = SRWLMagFldM(
                _G=_bx, _m=1, _n_or_s="s", _Leff=_len, _Ledge=_led, _R=_r
            )  # ?
        ...
        if _by != 0:
            dipBy = SRWLMagFldM(
                _G=_by, _m=1, _n_or_s="n", _Leff=_len, _Ledge=_led, _R=_r
            )
        ...
        if _gn != 0:
            quadN = SRWLMagFldM(
                _G=_gn, _m=2, _n_or_s="n", _Leff=_len, _Ledge=_led, _R=_r
            )
        ...
        if _gs != 0:
            quadS = SRWLMagFldM(
                _G=_gs, _m=2, _n_or_s="s", _Leff=_len, _Ledge=_led, _R=_r
            )
    ...
    # ------------------------------------------------------------------------
    def set_mag_kick(self, _angx=1.0, _angy=0, _len=1.0, _led=0, _zc=0, _add=0):
        ...
        if self.eBeam is None:
            raise Exception(
                "Electron Beam structure is not defined (it is required for defining kick magnet parameters from angle)"
            )
        ...
        if _angx != 0.0:
            kickMag = srwl_mag_kick(
                _el_en=elEn_GeV, _ang=_angx, _x_or_y="x", _len=_len, _led=_led
            )
        ...
        if _angy != 0.0:
            kickMag = srwl_mag_kick(
                _el_en=elEn_GeV, _ang=_angy, _x_or_y="y", _len=_len, _led=_led
            )
            self.mag_approx.arMagFld.append(kickMag)
            self.mag_approx.arXc.append(0)
            self.mag_approx.arYc.append(0)
    ...
    def set_und_tab_total(
        ...
        if _meas_or_calc == "m":
            if hasattr(self, "dir_magn_meas") and hasattr(self, "fn_magn_meas_sum"):
                fPathSum = os.path.join(
                    os.getcwd(),
                    self.dir_main,
                    self.dir_magn_meas,
                    self.fn_magn_meas_sum,
                )
            else:
                raise Exception("No magnetic measurements data are supplied")
        ...
        for i in range(nRows):
            ...
            if curLenLineParts >= 4:
                ...
                if curPhaseMode != _ph_mode:
                    continue

                arGaps.append(float(curLineParts[0]))

                curPhase = float(curLineParts[2])
                if (phasePrev is not None) and (curPhase != phasePrev):
                    phaseIsVar = True
                ...
                if len(curFileName) > 0:
                    curFilePath = os.path.join(
                        os.getcwd(), self.dir_main, self.dir_magn_meas, curFileName
                    )
                    # print(curFilePath)
        ...
        fldCnt = SRWLMagFldC(
            arMagFld3D, array("d", arXc), array("d", arYc), array("d", arZc)
        )
        ...
        if len(arCoefBx) == nElem:
            fldCnt.arPar3 = array("d", arCoefBx)
        if len(arCoefBy) == nElem:
            fldCnt.arPar4 = array("d", arCoefBy)
        ...
        if (_dg_by_len != 0.0) or (_y0 != 0.0) or (_yp != 0.0):
            self.mag.arMagFld[0] = srwl_und_fld_1d_mis(
                self.mag.arMagFld[0],
                _per,
                _dg_by_len,
                _c1,
    ...
    def set_und_tab(
        ...
        precPar = [
            ...
            _interp_ord,
            meshIsRect,
        ]  # NOTE: these values can be modified by srwl.UtiUndFindMagFldInterpInds

        # print(precPar)
        nIndReq = srwl.UtiUndFindMagFldInterpInds(
            arIndReqFiles, arGaps, arPhases, precPar
        )  # to implement
        # print(arIndReqFiles)
        ...
        for j in range(nIndReq):
            ...
            if curFldCnt is not None:
                ...
                if len(arCoefBx) > curInd:
                    arResCoefBx.append(arCoefBx[curInd])
                if len(arCoefBy) > curInd:
                    arResCoefBy.append(arCoefBy[curInd])

        fldResCnt = SRWLMagFldC(
            arResMagFld3D, array("d", arResXc), array("d", arResYc), array("d", arResZc)
        )
        # nElem = len(arMagFld3D)
        # if((nElem != len(arGaps)) or (nElem != len(arPhases))):
        #    raise Exception('Inconsistent magnetic field data summary file')
        ...
        if len(arResCoefBx) == nIndReq:
            fldResCnt.arPar3 = array("d", arResCoefBx)
        if len(arResCoefBy) == nIndReq:
            fldResCnt.arPar4 = array("d", arResCoefBy)
        ...
        fldCntFinRes = SRWLMagFldC(
            copy(arResMagFld3D[0]), arResXc[0], arResYc[0], arResZc[0]
        )

        precPar.append(
            1
        )  # this means that search for necessary subset of indexes should not be done, i.e. the field container is assumed to include only the fields necessary for the interpolaton
        # print(precPar)
    ...
    def set_und_per_from_tab(self, _rel_ac_thr=0.05, _max_nh=5, _max_per=0.1):
        ...
        if self.mag is None:
            raise Exception(sErMes)
        if isinstance(self.mag, SRWLMagFldC) == False:
            raise Exception(sErMes)
        ...
        srwl.UtiUndFromMagFldTab(
            self.mag_approx, self.mag, [_rel_ac_thr, _max_nh, _max_per]
        )
        return self.mag_approx
    ...
    def set_optics(self, _op, _v=None):  # OC28042018
        ...
        if _op is None:  # OC16122018
            if _v.op_func is None:
                raise Exception(
                    "Optics container structure (SRWLOptC) or function for setting it up is not defined"
                )
            else:
                _op = _v.op_func(
                    _v
                )  # assuming _v.op_func is reference to set_optics function of virtual beamline script
    ...
    # ------------------------------------------------------------------------
    def set_detector(
        self, _x=0, _rx=0, _nx=0, _dx=0, _y=0, _ry=0, _ny=0, _dy=0, _ord=1, _fname=""
    ):
        """Setup detector
        ...
        return SRWLDet(
            _xStart=_x - xHalfRangeDet,
            _xFin=_x + xHalfRangeDet,
            _nx=_nx,
            _yStart=_y - yHalfRangeDet,
            _yFin=_y + yHalfRangeDet,
            _ny=_ny,
            _dx=_dx,
            _dy=_dy,
            _spec_eff=detSpecEff,
            _eStart=eStartDetSpecEff,
            _eFin=eFinDetSpecEff,
        )
    ...
    def calc_el_trj(self, _ctst, _ctfi, _np=50000, _mag_type=1, _fname=""):
        ...
        if self.eBeam is None:
            raise Exception("Electron Beam structure is not defined")

        if _mag_type == 1:
            if self.mag_approx is None:
                raise Exception("Approximate Magnetic Field is not defined")
        elif _mag_type == 2:
            if self.mag is None:
                raise Exception("Magnetic Field is not defined")
        else:
            raise Exception("Incorrect Magnetic Field type identificator")
    ...
    # ------------------------------------------------------------------------
    def calc_int_from_wfr(
        self, _wfr, _pol=6, _int_type=0, _det=None, _fname="", _pr=True
    ):  # OC06042018
        ...
        if depType < 0:
            raise Exception("Incorrect numbers of points in the mesh structure")
    ...
    def calc_sr_se(
        ...
        zStartInteg = (
            _zi  # longitudinal position to start integration (effective if < zEndInteg)
        )
    ...
    def calc_rad_pt_src(
        ...
        wfr.partBeam.partStatMom1.xp = 0.0
        wfr.partBeam.partStatMom1.yp = 0.0
    ...
    def calc_ur_spec_me(
        ...
        if isinstance(und, SRWLMagFldU) == False:
            raise Exception("Incorrect SRWLMagFldU (i.e. undulator) structure")
        ...
        if len(_fname) > 0:
            ...
            srwl_uti_save_intens_ascii(
                ...
                _fname,
                0,
                ["Photon Energy", "Horizontal Position", "Vertical Position", sValName],
                _arUnits=["eV", "m", "m", sValUnitName],
            )
        return arI
    ...
    def calc_arb_spec_me(
        ...
        if _mag == 1:
            ...
            if self.mag is None:
                raise Exception("Accurate Magnetic Field is not defined")

        charMultiE = 0  # Calculate intensity (flux per unit surface by default)
        if _type == 1:
            charMultiE = 10  # Calculate flux
        ...
        stk = srwl_wfr_emit_prop_multi_e(
            ...
            _mesh=_mesh,
            _sr_meth=_meth,
            _sr_rel_prec=_rel_prec,
            _n_part_tot=_n_part_tot,
            _n_part_avg_proc=_n_part_avg_proc,
            _n_save_per=_n_save_per,
            _rand_meth=_rand_meth,
            # _file_path = _fname, _char = charMultiE)
            _file_path=_fname,
            _sr_samp_fact=_sr_samp_fact,
            _char=charMultiE,
            _det=_det,
            _me_approx=_me_approx,  # ) #OC14042018
    ...
    # ------------------------------------------------------------------------
    def calc_pow_den(
        self, _mesh, _prec=1, _meth=1, _z_start=0.0, _z_fin=0.0, _mag_type=1, _fname=""
    ):
        """Calculates multi-electron flux of undulator radiation (within fixed aperture of per unit surface), using approximate periodic magnetic field
        ...
        if self.eBeam is None:
            raise Exception(
                "Electron Beam structure is not defined"
            )  # AH03022019 (identified raise missing in many places)
    ...
    # ------------------------------------------------------------------------
    def calc_und_oper_tab(
        self, _mesh, _pol=0, _hi=1, _hf=1, _meas_or_calc="m", _zc=0, _fname=""
    ):
        ...
        nxPartIntegPowDens = 101
        nyPartIntegPowDens = 101
        ...
        for i in range(nRows):
            ...
            if curLenLineParts >= 4:
                ...
                if curLenLineParts >= 6:
                    ...
                    if ((curCoefBx != 1.0) or (curCoefBy != 1.0)) and (
                        curFldCnt is not None
                    ):
                ...
                if curFldCnt is not None:
                    ...
                    srwl.UtiUndFromMagFldTab(
                        undMagApprox,
                        curFldCnt,
                        [relAcThrConv2Per, maxNumHarmConv2Per, maxPerConv2Per],
                    )
                    ...
                    srwl.CalcStokesUR(
                        stkSpF,
                        self.eBeam,
                        undMagApprox.arMagFld[0],
                        [1, _hf + 5, longPrecStkF, azPrecStkF, 1],
                    )
                    ...
                    srwl.CalcElecFieldSR(
                        wfrSp, 0, curFldCnt, [methSR, relPrecSR, 0, 0, 20000, 1, -1]
                    )
                    ...
                    srwl.CalcIntFromElecField(
                        arSpecI, wfrSp, _pol, 0, 0, wfrSp.mesh.eStart, 0, 0
                    )
                    ...
                    stkP.mesh.zStart = (
                        curMeshF.zStart
                    )  # longitudinal position [m] at which power density has to be calculated
                    powHalfRangeX = (
                        2.0
                        * curMeshF.zStart
                        * undApprox.get_K()
                    ...
                    powHalfRangeY = (
                        2.0 * curMeshF.zStart / self.eBeam.partStatMom1.gamma
                    )
                    ...
                    srwl.CalcPowDenSR(
                        ...
                        0,
                        curFldCnt,
                        [precFactPow, methPow, 0, 0, 20000],
                    )
                    powTot = (
                        uti_math.integ_ar_2d(
                            stkP.arS,
                            1,
                            [stkP.mesh.xStart, stkP.mesh.xFin, stkP.mesh.nx],
                            [stkP.mesh.yStart, stkP.mesh.yFin, stkP.mesh.ny],
                        )
                        * 1.0e06
                    )
                    ...
                    powInAp = (
                        uti_math.integ_ar_2d(
                            ...
                            [stkP.mesh.xStart, stkP.mesh.xFin, stkP.mesh.nx],
                            [stkP.mesh.yStart, stkP.mesh.yFin, stkP.mesh.ny],
                            [curMeshF.xStart, curMeshF.xFin, nxPartIntegPowDens],
                            [curMeshF.yStart, curMeshF.yFin, nyPartIntegPowDens],
                        )
                        * 1.0e06
                    )
                    ...
                    for iHarmUR in range(_hi, _hf + 1, 1):
                        ...
                        ieStartSearch = 0
                        if (curEnStart > curMeshF.eStart) and (
                            curEnStart <= curMeshF.eFin
                        ):
                            ieStartSearch = int(
                                (curEnStart - curMeshF.eStart) / eStep + 1.0e-10
                            )
                        if ieStartSearch < 0:
                            ieStartSearch = 0
                        ...
                        if (curEnEnd > curMeshF.eStart) and (curEnEnd <= curMeshF.eFin):
                            ieEndSearch = int(
                                (curEnEnd - curMeshF.eStart) / eStep + 1.0e-10
                            )

                        if ieEndSearch >= curMeshF.ne:
                            ieEndSearch = curMeshF.ne - 1
                        if ieEndSearch < ieStartSearch:
                            ieEndSearch = ieStartSearch

                        curMaxFlux, iCurMaxFlux = uti_math.find_ar_max(
                            stkSpF.arS, ieStartSearch, ieEndSearch
                        )
                        enMaxFlux = curMeshF.eStart + eStep * iCurMaxFlux
                        enShift = enMaxFlux - e1Approx * iHarmUR
                        # arEnShift.append(enShift)

                        curMaxIntSE, iCurMaxIntSE = uti_math.find_ar_max(
                            arSpecI, ieStartSearch, ieEndSearch
                        )
                        ...
                        print(
                            "Estimated Photon Energy for Max. Flux:", enMaxFluxEst, "eV"
                        )
    ...
    def calc_wfr_prop(
        self, _wfr, _pres_ang=0, _pol=6, _int_type=0, _dep_type=3, _fname="", _det=None
    ):  # OC06122016
        ...
        if (hasattr(self, "optics") == False) or (
            isinstance(self.optics, SRWLOptC) == False
        ):
            raise Exception("Incorrect optics container (SRWLOptC) structure")
        ...
        if _pres_ang != 0:
            srwl.SetRepresElecField(_wfr, "a")
    ...
    def calc_wfr_emit_prop_me(
        ...
        if magToUse is None:
            if self.gsnBeam is not None:
                magToUse = (
                    self.gsnBeam
                )  # OC15092017 (because _mag is used for SRWLGsnBm when doing partially-coherent simulations in the scope of Gaussian-Schell model)
            elif self.ptSrc is not None:
                magToUse = (
                    self.ptSrc
                )  # OC16102017 (because _mag is used for SRWLPtSrc when doing partially-coherent simulations in the scope of Van-Cittert / Zernike model)
        ...
        return srwl_wfr_emit_prop_multi_e(
            ...
            _sr_meth=_sr_meth,
            _sr_rel_prec=_sr_rel_prec,
            _wr=_in_wr,
            _wre=_in_wre,  # OC05012017
            _n_part_tot=_n_part_tot,
            _n_part_avg_proc=_n_part_avg_proc,
            _n_save_per=_n_save_per,
            _file_path=_fname,
            ...
            _e_ph_integ=_e_ph_integ,
            _rand_meth=_rand_meth,
            _det=_det,
            _me_approx=_me_approx,
            _file_bkp=True if (_fbk == True) else False,
        )  # OC14082018
    ...
    def cost_func(self, _x, *_aux):
        ...
        if (nVars != len(nmVars)) or (nVars != len(x_lim)):
            raise Exception(
                "Inconsistent numbers of optimization parameters / names / limits"
            )

        if not self.uti_math_opt.check_limits(_x, x_lim):
            return 1.0e23  # to tune
        ...
        for i in range(nVars):
            ...
            setattr(
                v, nmVars[i], _x[i]
            )  # setting instant values of optimization variables
        ...
        if v.si or v.ws or v.wg:  # fully-coherent / single-electron calculations
            ...
            def critInf(_parDescr, _parName, _subCost):
                return (" " + _parDescr + "=" + frm).format(_parName) + (
                    " (sub-cost:" + frm + ")"
                ).format(_subCost)
            ...
            if (
                ...
                or (cw["xFWFM"] > 0)
                or (cw["yFWFM"] > 0)
            ):  # horizontal, vertical sizes, and peak intensity
                # if((cw[0] > 0) or (cw[1] > 0) or (cw[6] > 0)): #horizontal, vertical sizes, and peak intensity

                meshI = wfr.mesh
                arI = array(
                    "f", [0] * meshI.nx * meshI.ny
                )  # "flat" 2D array to take intensity data
                srwl.CalcIntFromElecField(
                    arI, wfr, 6, 0, 3, meshI.eStart, 0, 0
                )  # total intensity vs x and y; consider adding different polarizations

                arParInf = [0]
                if (cw["xFWFM"] > 0) or (cw["yFWFM"] > 0):
                    arParInf = [0, 0, 0]
                    if cw["xFWFM"] > 0:
                        arParInf[1] = v.om_ce["xFWFM"]
                    if cw["yFWFM"] > 0:
                        arParInf[2] = v.om_ce["yFWFM"]
                ...
                if (cw["xFWHM"] > 0) and (cn["xFWHM"] > 0):  # horizontal size
                    ...
                    if infIsReq:
                        s2pr += critInf("xFWHM", xFWHM, dCost)
                    cost += dCost
                ...
                if (cw["yFWHM"] > 0) and (cn["yFWHM"] > 0):  # vertical size
                    ...
                    if infIsReq:
                        s2pr += critInf("yFWHM", yFWHM, dCost)
                    cost += dCost

                if (cw["xFWFM"] > 0) and (
                    cn["xFWFM"] > 0
                ):  # additional horizontal size
                    xFWFM = infI[7]
                ...
                if (cw["yFWFM"] > 0) and (cn["yFWFM"] > 0):  # additional vertical size
                    ...
                    if infIsReq:
                        s2pr += critInf("yFWFM", yFWFM, dCost)
                    cost += dCost
                ...
                if (cw["iM"] > 0) and (cn["iM"] > 0):  # peak intensity
                    ...
                    if infIsReq:
                        s2pr += critInf("iM", maxI, dCost)
                    cost += dCost

            if (cw["xpFWHM"] > 0) or (
                cw["ypFWHM"] > 0
            ):  # horizontal and vertical angular divergences
                ...
                if (cw["xpFWFM"] > 0) or (cw["ypFWFM"] > 0):
                    arParInf = [0, 0, 0]
                    if cw["xpFWFM"] > 0:
                        arParInf[1] = v.om_ce["xpFWFM"]
                    if cw["ypFWFM"] > 0:
                        arParInf[2] = v.om_ce["ypFWFM"]
                ...
                if (cw["xpFWHM"] > 0) and (
                    ...
                    if infIsReq:
                        s2pr += critInf("xpFWHM", xpFWHM, dCost)
                    cost += dCost
        ...
        if (
            v.wm
        ):  # partially-coherent / multi-electron calculations (add treatment of the Gaussian-Schell case later)
            ...
            if (
                len(v.wm_fni) > 0
            ):  # load Intensity / Degree of Coherence from files and analyze the data

                if ((cw["xFWHM"] > 0) or (cw["yFWHM"] > 0) or (cw["iM"] > 0)) and (
                    (v.wm_ap == 0) or (v.wm_ap == 2)
                ):  # horizontal, vertical sizes and peak intensity
                    if (v.wm_ch == 0) or (v.wm_ch == 40):
                        arI, meshI = srwl_uti_read_intens_ascii(
                            os.path.join(os.getcwd(), v.fdir, v.wm_fni), "f"
                        )
                        ...
                        if (cw["xFWFM"] > 0) and (
                            ...
                            dCost = cw["xFWFM"] * rx * rx
                            if infIsReq:
                                s2pr += critInf("xFWFM", xFWFM, dCost)
                            cost += dCost

                        if (cw["yFWFM"] > 0) and (
                            cn["yFWFM"] > 0
                        ):  # additional vertical size
                            yFWFM = infI[8]
                            ry = (yFWFM - ct["yFWFM"]) / cn["yFWFM"]
                            dCost = cw["yFWFM"] * ry * ry
                            if infIsReq:
                ...
                if ((cw["xpFWHM"] > 0) or (cw["ypFWHM"] > 0)) and (
                    (v.wm_ap == 1) or (v.wm_ap == 2)
                ):  # horizontal and vertical angular divergences
                    # if((v.wm_ch == 0) or (v.wm_ch == 40)):
                    fnAngI = v.wm_fni
                    if v.wm_ap == 2:
                        fnAngI = srwl_wfr_fn(v.wm_fni, 2)
                    arI, meshI = srwl_uti_read_intens_ascii(
                        os.path.join(os.getcwd(), v.fdir, fnAngI), "f"
                    )
                    ...
                    infI = srwl.UtiIntInf(
                        arI, meshI, arParInf
                    )  # estimate FWHMs, starting search from extremities of the distribution
                ...
                if ((cw["xCL"] > 0) or (cw["yCL"] > 0)) and (
                    (v.wm_ch == 4) or (v.wm_ch == 40)
                ):  # horizontal and vertical coherence lengths

                    if (cw["xCL"] > 0) and (
                        cn["xCL"] > 0
                    ):  # horizontal coherence length
                        fnDC = srwl_wfr_fn(v.wm_fni, 41)
                        arDC, meshDC = srwl_uti_read_intens_ascii(
                            os.path.join(os.getcwd(), v.fdir, fnDC), "f"
                        )
                        infDC = srwl.UtiIntInf(
                            arDC, meshDC, [1]
                        )  # estimate FWHMs, starting search from center of the distribution
                        xCoh = infDC[5]  # cut vs (x1+x2)/2
                        rx = (xCoh - ct["xCL"]) / cn["xCL"]
                        dCost = cw["xCL"] * rx * rx
                        if infIsReq:
                            s2pr += critInf("xCL", xCoh, dCost)
                        cost += dCost
        ...
        if v.om_pr or v.om_pr:
            ...
            if v.om_pr:
                print(statStr)
            if v.om_fl:
                self.uti_math_opt.log_update(v.om_fnl, statStr)
    ...
    def calc_all(self, _v, _op=None):  # 16122018
        ...
        if (
            _v.om
        ):  # since the optimization modifies params in _v and calls calc_all, it goes first
            try:  # 16122018
                import uti_math_opt
                ...
                traceback.print_exc()
                print(
                    "Optimization can not be performed (probably because required libraries are missing)."
                )
                return
            ...
            om_cw_vals = uti_math_opt.norm_weights(
                [_v.om_cw[k] for k in _v.om_cw.keys()]
            )
            ik = 0
            ...
            if _v.om_fl:
                _v.om_fnl = self.uti_math_opt.log_init("__srwl_logs__")

            _v.om_res = [0] * (
                len(_v.om_iv) + 2
            )  # array for storing instant optimization results in the process of the optimization
            for ip in range(len(_v.om_iv)):
                _v.om_res[ip] = _v.om_iv[ip]

            uti_math_opt.minimize(
                self.cost_func,
                _v.om_iv,
                _x_lim=_v.om_lm,
                _meth=_v.om_mt,
        ...
        if hasattr(_v, "rs_type"):  # OC29012019
            ...
            if (_v.rs_type != "u") and (
                _v.rs_type != "c"
            ):  # not idealized undulator / multi-pole wiggler (and e-beam)
                if hasattr(_v, "und_b"):
                    _v.und_b = 0
                if hasattr(_v, "und_by"):
                    _v.und_by = 0
                if hasattr(_v, "und_bx"):
                    _v.und_bx = 0

            if (_v.rs_type != "t") and (
                _v.rs_type != "c"
            ):  # not ID with tabulated magnetic field, including dependence on gap / phase (and e-beam)
                if hasattr(_v, "und_g"):
                    del _v.und_g

            if (_v.rs_type != "d") and (
                _v.rs_type != "c"
            ):  # not idealized dipole magnet with eventual gradient (and e-beam)
                if hasattr(_v, "mag_bx"):
                    _v.mag_bx = 0
                if hasattr(_v, "mag_by"):
                    _v.mag_by = 0
                if hasattr(_v, "mag_gn"):
                    _v.mag_gn = 0
                if hasattr(_v, "mag_gs"):
                    _v.mag_gs = 0

            if (_v.rs_type != "a") and (
                _v.rs_type != "c"
            ):  # not tabulated magnetic field not depending on gap / phase, i.e. arbitrary magnetic field source (and e-beam)
        ...
        # if hasattr(_v, 'und_b'):
        if (
            hasattr(_v, "und_b") or hasattr(_v, "und_by") or hasattr(_v, "und_bx")
        ):  # OC25052016
            if hasattr(_v, "und_bx") == False:
                _v.und_bx = 0
            if hasattr(_v, "und_by") == False:
                _v.und_by = _v.und_b
            if hasattr(_v, "und_phx") == False:
                _v.und_phx = 0
            if hasattr(_v, "und_phy") == False:
                _v.und_phy = 0
            if hasattr(_v, "und_zc") == False:
                _v.und_zc = 0
            ...
            if _v.und_b2e:
                e1 = self.mag_approx.arMagFld[len(self.mag_approx.arMagFld) - 1].get_E1(
                    _en_elec=self.eBeam.partStatMom1.get_E(), _unit="eV"
                )
                print(
                    "Fundamental Photon Energy:", srwl_uti_num_round(e1), "eV"
                )  # check how it will work under IPython

            if _v.und_e2b:
                b = self.mag_approx.arMagFld[len(self.mag_approx.arMagFld) - 1].E1_2_B(
                    _e1=_v.w_e, _en_elec=self.eBeam.partStatMom1.get_E()
                )
                print(
                    "Magnetic Field Amplitude:", srwl_uti_num_round(b), "T"
                )  # check how it will work under IPython

        if (
            hasattr(_v, "und2_b") or hasattr(_v, "und2_by") or hasattr(_v, "und2_bx")
        ):  # OC03122016
            if not hasattr(_v, "und2_bx"):
                _v.und2_bx = 0
            if not hasattr(_v, "und2_by"):
                _v.und2_by = _v.und2_b
            if not hasattr(_v, "und2_phx"):
                _v.und2_phx = 0
            if not hasattr(_v, "und2_phy"):
                _v.und2_phy = 0
            if not hasattr(_v, "und2_zc"):
                _v.und2_zc = 0
            if (_v.und2_bx != 0) or (_v.und2_by != 0):
                ...
                if (_v.und2_cma != 0) and (
                    _v.und2_cml > 0
                ):  # setting-up canting magnet (to move to a separate function and make more general; treat soft edges ensuring corrct kick angle)
                    self.set_mag_kick(
                        _angx=_v.und2_cma,
                        _angy=0,
                        _len=_v.und2_cml,
        ...
        if hasattr(_v, "und_mdir"):
            ...
            if len(self.dir_magn_meas) > 0:
                magnMeasDirExists = True
        ...
        if magnMeasSumFileExists and hasattr(_v, "und_g"):
            if _v.und_g > 0.0:
                phase = 0.0
                if hasattr(_v, "und_ph"):
                    phase = _v.und_ph
                phase_mode = "p1"
                if hasattr(_v, "und_phm"):
                    phase_mode = _v.und_phm
                ...
                self.set_und_tab(  # setup undulator source from measured magnetic field data
                    ...
                    _y0=_v.ebm_y
                    + _v.ebm_yp * _v.und_zc
                    - _v.und_dy,  # this assumes that e-beam parameters are defined at z=0
                    _yp=_v.ebm_yp - _v.und_yp,
                )

                # if((_v.ss_mag == 1) or (_v.ss_mag == 1) or (_v.w_mag == 1) or (_v.tr_mag == 1)):
                if (
                    (_v.ss and (_v.ss_mag == 1))
                    or (_v.sm and (_v.sm_mag == 1))
                    or ((_v.ws or _v.wm) and (_v.w_mag == 1))
                    or (_v.tr and (_v.tr_mag == 1))
                ):
                    # print('test field conversion')
                    maxPer = _v.und_per + 0.01
                    self.set_und_per_from_tab(
                        _rel_ac_thr=0.05, _max_nh=7, _max_per=maxPer
                    )
        ...
        if hasattr(_v, "mag_bx") == False:
            _v.mag_bx = 0
        if hasattr(_v, "mag_by") == False:
            _v.mag_by = 0
        if hasattr(_v, "mag_gn") == False:
            _v.mag_gn = 0
        if hasattr(_v, "mag_gs") == False:
            _v.mag_gs = 0
        if hasattr(_v, "mag_len") == False:
            _v.mag_len = 1.5  # ?
        if hasattr(_v, "mag_led") == False:
            _v.mag_led = 0
        if hasattr(_v, "mag_r") == False:
            _v.mag_r = 0
        if hasattr(_v, "mag_zc") == False:
            _v.mag_zc = 0
        ...
        if hasattr(_v, "mag_fn"):
            if len(_v.mag_fn) > 0:
                ...
                if hasattr(self, "dir_main"):
                    if len(self.dir_main) > 0:
                        magPath = os.path.join(magPath, self.dir_main)
                if hasattr(_v, "mag_mdir"):
                    if len(_v.mag_mdir) > 0:
                        magPath = os.path.join(magPath, _v.mag_mdir)
                ...
                if not os.path.exists(magPath):  # AH03022019
                    raise Exception(
                        "Magnetic field file {} can not be found".format(_v.mag_fn)
                    )
                ordInterp = 1
                if hasattr(_v, "mag_ior"):
                    ordInterp = _v.mag_ior
                self.set_mag_tab(  # setup magnet from tabulated magnetic field data file
                    _fpath=magPath, _zc=_v.mag_zc, _interp_ord=ordInterp
                )
        ...
        detector = None
        if (
            (_v.d_rx > 0.0) and (_v.d_nx > 0) and (_v.d_ry > 0.0) and (_v.d_ny > 0)
        ):  # OC06122016
        ...
        if _v.ss or _v.gs:
            ...
            srCanBeCalc = (self.eBeam is not None) and (
                (self.mag_approx is not None) or (self.mag is not None)
            )
            # print("                    _v.ss=", _v.ss)
            # print("                    _v.gs=", _v.gs)
            ...
            if (_v.gs != True) and (srCanBeCalc == True):
                ...
                _v.w_res, _v.ss_res, mesh_dummy = self.calc_sr_se(  # OC16102017
                    ...
                    _int_type=0,
                    _mag_type=_v.ss_mag,
                    _fname=os.path.join(_v.fdir, _v.ss_fn)
                    if (len(_v.ss_fn) > 0)
                    else "",
                )

            if (_v.gs == True) or (
                (self.gsnBeam is not None) and (srCanBeCalc == False)
            ):
        ...
        if _v.sm:
            mesh_sm = SRWLRadMesh(
                ...
                _v.sm_ef,
                _v.sm_ne,
                _v.sm_x - 0.5 * _v.sm_rx,
                _v.sm_x + 0.5 * _v.sm_rx,
                _v.sm_nx,
                _v.sm_y - 0.5 * _v.sm_ry,
                _v.sm_y + 0.5 * _v.sm_ry,
                _v.sm_ny,
                _v.op_r,
            )
            if (_v.sm_mag == 1) and (_v.sm_meth < 0):
                ...
                _v.sm_res = self.calc_ur_spec_me(  # OC16102017
                    ...
                    _type=_v.sm_type,
                    _pol=_v.sm_pol,
                    _fname=os.path.join(_v.fdir, _v.sm_fn)
                    if (len(_v.sm_fn) > 0)
                    else "",
                )
        ...
        if _v.pw:
            mesh_pw = SRWLRadMesh(
                1000,
                1000,
                1,  # dummy (not used for power density)
                _v.pw_x - 0.5 * _v.pw_rx,
                _v.pw_x + 0.5 * _v.pw_rx,
                _v.pw_nx,
                _v.pw_y - 0.5 * _v.pw_ry,
                _v.pw_y + 0.5 * _v.pw_ry,
                _v.pw_ny,
                _v.op_r,
            )
        ...
        if _v.si or _v.ws or _v.gi or _v.wg or _v.wm:
            ...
            if _v.si or _v.ws or _v.gi or _v.wg:
                ...
                if hasattr(self, "ptSrc"):
                    ptSrcSphWaveCanBeCalc = self.ptSrc is not None
                ...
                if len(_v.ws_fnei) > 0:
                    ...
                    if _v.w_res is not None:
                        _v.si_res, mesh_si = self.calc_int_from_wfr(
                            _v.w_res, _v.si_pol, _v.si_type, detForSI, _pr=False
                        )
                        wfrWasNotLoaded = False
                        print("completed (lasted", round(time.time() - t0, 2), "s)")
                    else:
                        print("failed to load wavefront file")
                ...
                if wfrWasNotLoaded:
                    ...
                    if (_v.gi != True) and (_v.wg != True) and (srCanBeCalc == True):
                        ...
                        _v.w_res, _v.si_res, mesh_si = self.calc_sr_se(  # OC16102017
                            ...
                            _mag_type=_v.w_mag,
                            _fname=os.path.join(_v.fdir, _v.si_fn)
                            if (len(_v.si_fn) > 0)
                            else "",
                            _det=detForSI,
                        )
                    ...
                    if (
                        (_v.gs == True)
                        or (_v.wg == True)
                        or ((gsnBeamCanBeCalc == True) and (srCanBeCalc == False))
                    ):  # OC01062016
                    ...
                    if (
                        (ptSrcSphWaveCanBeCalc is True)
                        and (gsnBeamCanBeCalc is not True)
                        and (srCanBeCalc is not True)
                    ):  # OC11102017
                        ...
                        (
                            ...
                            _v.si_res,
                            mesh_si,
                        ) = self.calc_rad_pt_src(  # OC16102017
                            _mesh=deepcopy(mesh_w),
                            _samp_fact=_v.w_smpf,
                            _pol=_v.si_pol,
                            _int_type=_v.si_type,
                ...
                if _v.ws or _v.wg:
                    ...
                    if _v.w_wr != 0.0:  # OC26032016
                        # wfr.Rx = _v.w_wr
                        # wfr.Ry = _v.w_wr
                        _v.w_res.Rx = _v.w_wr  # OC16102017
                        _v.w_res.Ry = _v.w_wr

                    if _v.w_wre > 0.0:  # OC05012017
                        # wfr.dRx = _v.w_wre
                        # wfr.dRy = _v.w_wre
                    ...
                    _v.ws_res, mesh_ws = self.calc_wfr_prop(  # OC16102017
                        ...
                        _fname=os.path.join(_v.fdir, _v.ws_fni)
                        if (len(_v.ws_fni) > 0)
                        else "",
                        _det=detector,
                    )
            ...
            if _v.wm:
                ...
                res_ipm = self.calc_wfr_emit_prop_me(
                    ...
                    _rand_meth=_v.wm_rm,
                    _fname=os.path.join(_v.fdir, _v.wm_fni)
                    if (len(_v.wm_fni) > 0)
                    else None,
                    _det=detector,
                    # _multi_e_approx = _v.wm_am)
                    _me_approx=_v.wm_am,  # ) #OC13042018
                    _fbk=True if (_v.wm_fbk) else False,
                )  # OC14082018
        ...
        if (_v.tr) and (
            len(_v.tr_pl) > 0
        ):  # OC06042018 (to make sure that it starts when _v.tr = 1)
            ...
            for i in range(len(arOpt)):
                ...
                if len(curOpt) > 0:
                    ...
                    if curOpt == "xz":
                        ...
                        if not bxExists:
                            raise ValueError(
                                "Horizontal magnetic field seen by electron was not calculated"
                            )
                    elif curOpt == "byz":
                        byExists = False
                        if hasattr(trj, "arBy"):
                        ...
                        if not byExists:
                            raise ValueError(
                                "Vertical magnetic field seen by electron was not calculated"
                            )
                    ...
                    if (arPlotAbsc is not None) and (arPlotOrd is not None):
                        if gridIsReg:
                            uti_plot1d(
                                arPlotOrd,
                                [min(arPlotAbsc), max(arPlotAbsc), len(arPlotAbsc)],
                                labels,
                                units,
                            )
                        else:
                            uti_plot1d_ir(arPlotOrd, arPlotAbsc, labels, units)
                    else:
                        raise ValueError(
                            "This trajectory plot option value is not supported: {}. The supported values are: xz,xpz,yz,ypz,yx,ypxp,bxz,byz".format(
                                curOpt
                            )
                        )
        ...
        if (_v.ss or _v.gs) and (len(_v.ss_pl) > 0):
            ...
            uti_plot1d(
                _v.ss_res,
                [mesh_ss.eStart, mesh_ss.eFin, mesh_ss.ne],
                [sArgLabel, sValLabel, sValLabel],
                [sArgUnit, sValUnit],
            )  # OC16102017
        ...
        if (_v.pw == True) and (len(_v.pw_pl) > 0):
            if (
                ...
                uti_plot1d(
                    _v.pw_res,
                    [mesh_pw.xStart, mesh_pw.xFin, mesh_pw.nx],  # OC16102017
                ...
                uti_plot1d(
                    _v.pw_res,
                    [mesh_pw.yStart, mesh_pw.yFin, mesh_pw.ny],  # OC16102017
...
def srwl_uti_std_options():
    ...
    varParamStd = [
        ...
        ],  # OC: re-check the default values
        ...
        ["und2_len", "f", 3.0, "undulator length [m]"],
        [
            "und_b",
            "f",
            0.0,
            "undulator vertical peak magnetic field [T]",
        ],  # Keeping it 0 here is important for parsing calculation options!
        [
            "und2_b",
            "f",
            0.0,
        ...
        ["und2_bx", "f", 0.0, "undulator horizontal peak magnetic field [T]"],
        ...
        [
            ...
            "f",
            0.0,
            "second undulator gap [mm] (assumes availability of magnetic measurement or simulation data)",
        ],
        [
            "und_ph",
            "f",
            0.0,
            "undulator phase, i.e. longitudinal shift of magnet arrays [mm] (assumes availability of magnetic measurement or simulation data)",
        ],
        [
            "und2_ph",
            "f",
            0.0,
            "second undulator phase, i.e. longitudinal shift of magnet arrays [mm] (assumes availability of magnetic measurement or simulation data)",
        ],
        ...
        ["und2_zc", "f", 0.0, "second undulator center longitudinal position [m]"],
        ["und2_cma", "f", 0.0, "canting magnet angle [rad]"],
        ["und2_cmz", "f", 0.0, "canting magnet longitudinal position [m]"],
        ["und2_cml", "f", 0.0, "canting magnet effective length [m]"],
        ["und2_cmd", "f", 0.0, "canting magnet edge length [m]"],
        [
            "und_b0",
            "f",
            0.0,
            "constant defining (approximate) undulator field dependence on gap (i.e. b0 in b0*exp(-c1*gap/per + c2*(gap/per)^2))",
        ],
        [
            "und2_b0",
            "f",
        ...
        [
            ...
            "f",
            0.0,
            "constant defining (approximate) undulator field dependence on gap (i.e. c1 in b0*exp(-c1*gap/per + c2*(gap/per)^2))",
        ],
        [
            "und2_c1",
            "f",
        ...
        [
            ...
            "f",
            0.0,
            "constant defining (approximate) undulator field dependence on gap (i.e. c2 in b0*exp(-c1*gap/per + c2*(gap/per)^2))",
        ],
        [
            "und2_c2",
            "f",
        ...
        [
            ...
            "f",
            0.0,
            "constant defining (approximate) undulator field dependence on vertical position (i.e. a in cosh(2*Pi*a*y/per)",
        ],
        [
            "und2_a",
            "f",
        ...
        [
            ...
            "f",
            0.0,
            "undulator gap taper, i.e. gap difference between exit and entrance [m]",
        ],
        [
            "und2_dg",
            "f",
        ...
        [
            ...
            "f",
            0.0,
            "undulator elevation in vertical direction over the median plane [m]",
        ],
        [
            "und2_dy",
            "f",
        ...
        [
            ...
            "f",
            0.0,
            "undulator vertical angular misalignment over the median plane [rad]",
        ],
        [
            "und2_yp",
            "f",
        ...
        [
            ...
            "s",
            "",
            "name of undulator magnetic measurements for different gaps summary file",
        ],
        [
            "und2_mfs",
            "s",
            "",
            "name of second undulator magnetic measurements for different gaps summary file",
        ],
        [
            "und_b2e",
            "",
        ...
        ["mag_by", "f", 0.0, "vertical magnetic field [T]"],
        ["mag_bx", "f", 0.0, "horizontal magnetic field [T]"],
        ["mag_bn", "f", 0.0, "normal magnetic field quadrupole [T/m]"],
        ["mag_bs", "f", 0.0, "skew magnetic field quadrupole [T/m]"],
        ["mag_len", "f", 3.0, "magnetic length [m]"],
        [
            "mag_led",
            "f",
            0.0,
            "magnetic edge length bw 10% and 90% of peak field [m]; G/(1 + ((z-zc)/d)^2)^2 field dependence is assumed",
        ],
        [
            "mag_r",
            "f",
            0.0,
            "radius of curvature of central trajectory [m] (for simulating e.g. quadrupole component integrated to a bending magnet; effective if > 0)",
        ],
        ...
        [
            ...
            "f",
            3,
            "interpolation order to use for tabulated magnetic field (1<= mag_ior <= 3)",
        ],
        ...
        [
            ...
            "f",
            0.0,
            "energy per pulse [J] (needs to be >0 for the Gaussian beam to be set up)",
        ],
        ...
        [
            ...
            "f",
            0.0,
            "initial longitudinal position along electron trajectory for SR spectrum calculation (effective if ss_zi < ss_zf)",
        ],
        [
            "ss_zf",
            "f",
            0.0,
            "final longitudinal position along electron trajectory for SR spectrum calculation (effective if ss_zi < ss_zf)",
        ],
        [
            "ss_pol",
            "i",
        ...
        [
            ...
            "",
            "",
            'calculate Gaussian beam spectrum vs photon energy or time (has priority over "ss" if both Gaussian beam and e-beam + magnetic field are defined)',
            "store_true",
        ],
        ...
        [
            ...
            "f",
            -1,
            "sampling factor for calculation of intensity distribution vs horizontal and vertical position (active if >0)",
        ],
        [
            "sm_mag",
            "i",
        ...
        [
            ...
            "i",
            1,
            "initial UR spectral harmonic to be taken into accountfor multi-e spectrum vs photon energy calculation",
        ],
        [
            "sm_hf",
            "i",
            15,
            "final UR spectral harmonic to be taken into accountfor multi-e spectrum vs photon energy calculation",
        ],
        [
            "sm_prl",
            "f",
        ...
        [
            ...
            "",
            "",
            "create backup file(s) with multi-e spectrum (only is it is calculated using the macro-particle method)",
            "store_true",
        ],
        ...
        [
            ...
            "f",
            0.0,
            "initial longitudinal position along electron trajectory for power density calculation (effective if pw_zi < pw_zf)",
        ],
        [
            "pw_zf",
            "f",
            0.0,
            "final longitudinal position along electron trajectory for power density calculation (effective if pw_zi < pw_zf)",
        ],
        [
            "pw_mag",
            "i",
        ...
        [
            ...
            "s",
            "und_oper_table.dat",
            "file name for saving calculated undulator operation table data",
        ],
        ...
        [
            ...
            "",
            "",
            'calculate coherent Gaussian beam intensity distribution (without wavefront propagation through a beamline) vs horizontal and vertical position (has priority over "si" if both Gaussian beam and e-beam + magnetic field are defined)',
            "store_true",
        ],
        ...
        [
            ...
            "",
            "",
            'calculate coherent Gaussian beam wavefront propagation (has priority over "si" if both Gaussian beam and e-beam + magnetic field are defined)',
            "store_true",
        ],
        ...
        [
            ...
            "f",
            0.0,
            "initial longitudinal position [m] along electron trajectory for SR calculation (effective if w_zi < w_zf)",
        ],
        [
            "w_zf",
            "f",
            0.0,
            "final longitudinal position [m] along electron trajectory for SR calculation (effective if w_zi < w_zf)",
        ],
        [
            "w_mag",
            "i",
        ...
        [
            ...
            "f",
            0.0,
            "wavefront radius to set (is taken into account if != 0) [m]; this parameter may be important for subsequent wavefront propagation simulations; by default, it is set by a function calculating the initial wavefront; however, it can also be set manually using this variable",
        ],
        [
            "w_wre",
            "f",
            0.0,
            "wavefront radius error (is taken into account if != 0) [m]; this parameter may be important for subsequent wavefront propagation simulations; by default, it is set by a function calculating the initial wavefront; however, it can also be set manually using this variable",
        ],
        [
            "si_pol",
            "i",
        ...
        [
            ...
            "s",
            "",
            "file name for saving initial electric field wavefront (before propagation)",
        ],
        [
            "ws_fnep",
            "s",
            "",
            "file name for saving electric field wavefront after propagation",
        ],
        [
            "ws_fnei",
            "s",
            "",
            "file name for input electric field for further eventual propagation; if this file name is supplied, the initial electric field will not be calculated, but loaded from this file",
        ],
        [
            "ws_fni",
            "s",
        ...
        [
            ...
            "s",
            "xy",
            'plot the propagated radiaiton intensity distributions in graph(s): ""- dont plot, "x"- vs horizontal position, "y"- vs vertical position, "xy"- vs horizontal and vertical position',
        ],
        [
            "ws_ap",
            "i",
            0,
            "switch specifying representation of the resulting Stokes parameters (/ Intensity distribution): coordinate (0) or angular (1)",
        ],
        [
            "si_pl",
            "s",
        ...
        [
            ...
            "i",
            5,
            "number of macro-electrons (coherent wavefronts) to average on each node at parallel (MPI-based) calculation of multi-electron wavefront propagation",
        ],
        [
            "wm_ns",
            "i",
        ...
        [
            ...
            "f",
            0.0,
            "length of drift space to be applied after propagation through a beamline [m]",
        ],
        # Detector parameters
        ["d_x", "f", 0.0, "central horizontal position [m] of detector active area"],
        [
            "d_rx",
            "f",
            0.0,
            "range of horizontal position [m] of detector active area (should be >0 to be taken into account)",
        ],
        [
            "d_nx",
            "i",
            0,
            "number of pixels vs horizontal position (should be >0 to be taken into account)",
        ],
        ["d_dx", "f", 0.0, "horizontal size of pixel [m]"],
        ["d_y", "f", 0.0, "central vertical position [m] of detector active area"],
        [
            "d_ry",
            "f",
            0.0,
            "range of vertical position [m] of detector active area (should be >0 to be taken into account)",
        ],
        [
            "d_ny",
            "i",
            0,
            "number of pixels vs vertical position (should be >0 to be taken into account)",
        ],
        ["d_dy", "f", 0.0, "vertical size of pixel [m]"],
        [
            "d_or",
            "f",
            1,
            "interpolation order (i.e. order of polynomials to be used at 2D interpolation)",
        ],
        [
            "d_ifn",
            "s",
            "",
            "file name with detector spectral efficiency data (on 1D mesh vs photon energy: _eStart, _eFin, _ne)",
        ],
        ...
        [
            "om_pn",
            "",
            ["op_VFM_r", "op_HFM_r", "op_S0_Dy", "op_S0H_Dx"],
            "names of parameters to be optimized",
        ],
        [
            "om_iv",
            "f",
            [20000.0, 20000.0, 1e-05, 1e-05],
            "initial values of parameters to be optimized",
        ],
        [
            "om_pb",
            "f",
            ...
            "bounding (i.e. min. amd max. possible) values of parameters to be optimized",
        ],
        [
            "om_cw",
            "f",
            [1, 1, 0, 0, 0, 0, 0],
            "weights of different criterions the optimization should be performed for: [0]- horizontal spot size, [1]- vertucal spot size, [2]- horizontal angular divergence, [3]- vertical angular divergence, [4]- horizontal coherence length, [5]- vertical coherence length, [6]- given intensity distribution",
        ],
        [
            "om_cn",
            "f",
            [1e-06, 1e-06, 0, 0, 0, 0, 0],
            "nominal values of different criterions / figures of merit for the optimization, in the same order as defined by om_cw",
        ],
...
def srwl_uti_parse_options_obs(
    _descr, use_sys_argv=True, args=None
):  # OC08032016 #MR26022016, MR04032016
    ...
    for i in range(nOpt):
        ...
        optIsList = False
        if isinstance(defVal, list) or isinstance(defVal, array):
            optIsList = True
    ...
    for i in range(len(listOptNamesPostParse)):
        ...
        if (isinstance(valCurOpt, list) == False) and (
            isinstance(valCurOpt, array) == False
        ):
            parsedVal = srwl_uti_parse_str2list(valCurOpt)
            setattr(v, curOptName, parsedVal)
...
def srwl_uti_parse_options(
    ...
    for i in range(nOpt):
        ...
        if len(sTypeShort) <= 0:
            p.add_argument(
                "--" + curOpt[0], default=defVal, help=curOpt[3], action=sAct
            )
        else:
            p.add_argument(
                "--" + curOpt[0],
    ...
    for i in range(len(listOptNamesPostParse)):
        ...
        if not isinstance(valCurOpt, list) and not isinstance(
            valCurOpt, array
        ):  # MR07032016
            parsedVal = srwl_uti_parse_str2list(valCurOpt)
            setattr(v, curOptName, parsedVal)
...
def setup_source(v):  # MR20160617 - moved from Sirepo .jinja template
    ...
    if v.source_type in ["u", "t"]:
        ...
        mag.arYc.append(0)
        mag.arMagFld.append(
            SRWLMagFldM(v.mp_field, v.mp_order, v.mp_distribution, v.mp_len)
        )
