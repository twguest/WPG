# -*- coding: utf-8 -*-
"""
Created on Sat Aug 31 14:02:34 2019
...
def calc_coherence(U, axisName, axis, p=0):
    """
    ...
    *p*: float, distance to screen
    ...
    print(Uc.shape)
    J = np.dot(Uc, Uc.T.conjugate()) / repeats
    print(J.shape)
    print(np.max(J))
    ...
    Jd = np.abs(np.diag((J)))  # DoC as the cross-diagonal
    print(Jd)
...
def calc_coherence_gsm(U, axisName, axis, p=0):
    """
    Calculates 1D degree of coherence (DoC). From its width in respect to the
    width of intensity distribution also infers the coherent fraction. Both
    widths are rms. The one of intensity is calculated over the whole axis, the
...
def plot_1D_degree_of_coherence(
    data1D,
    axisName,
    axis,
    unit="mm",
    fig2=None,
    isIntensityNormalized=False,
    locLegend="auto",
):
    """
    ...
    *locLegend*: str, legend location in matplotlib style.wpg_uti_coherence
    ...
    ax.imshow(
        np.abs(Jx), origin="upper", extent=[-am, am, am, -am], interpolation="none"
    )
    ...
    if fig2 is None:
        ...
        if isIntensityNormalized:
            ...
            ax2.set_ylabel("intensity (ph/s/{0}²/0.1%BW)".format(unit), color="C0")
...
def plot_eigen_modes(x, y, w, v, xlabel="", ylabel=""):
    ...
    *w* and *v* returned by :func:`calc_eigen_modes_PCA` or
    ...
    rect2d = [
        xMargin / xFigSize,
        (yMargin + dY + space) / yFigSize,
        dX / xFigSize,
        dY / yFigSize,
    ]
    ax0 = figMs.add_axes(rect2d, aspect=1)
    rect2d = [
        (xMargin + dX + space) / xFigSize,
        (yMargin + dY + space) / yFigSize,
        dX / xFigSize,
        dY / yFigSize,
    ]
    ax1 = figMs.add_axes(rect2d, aspect=1)
    rect2d = [xMargin / xFigSize, yMargin / yFigSize, dX / xFigSize, dY / yFigSize]
    ...
    modeName = "mode"
    labels = [
        "0th (coherent) {0}: w={1:.3f}",
        "1st residual {0}: w={1:.3f}",
        "2nd residual {0}: w={1:.3f}",
        "3rd residual {0}: w={1:.3f}",
    ]
    ...
    if xlabel:
        figMs.text(
            (xMargin + dX + space / 2.0) / xFigSize,
            yMargin * 0.3 / yFigSize,
            xlabel,
            ha="center",
            va="center",
        )
    if ylabel:
        figMs.text(
            xMargin * 0.3 / xFigSize,
            (yMargin + dY + space / 2.0) / yFigSize,
            ylabel,
            ha="center",
            va="center",
            rotation="vertical",
...
if __name__ == "__main__":
    ...
    eigenval = np.loadtxt(r"./extensions/out/eigen/eigenval.csv", delimiter=",")
    ...
    for itr in range(3):
        if itr == 0:
            wfr = Wavefront()
            wfr.load_hdf5(
                r"/nfs/data/users/twg/gsmProp/atSource/wfr_mode_{}.hdf5".format(itr)
            )
            eField = wfr.data.arrEhor
        else:
            wfr = Wavefront()
            wfr.load_hdf5(
                r"/nfs/data/users/twg/gsmProp/atSource/wfr_mode_{}.hdf5".format(itr)
            )
            eField += wfr.data.arrEhor * eigenval[itr]
            print(eigenval[itr])
    axis = np.linspace(wfr.params.Mesh.xMin, wfr.params.Mesh.xMax, wfr.params.Mesh.nx)
    data = calc_coherence(eField[:, :, 0, :], axisName="x", axis=axis)
    print(data[3] ** 0.5, data[4] ** 0.5)
