...
class Backend(object):
    ...
    def uti_plot1d_ir(
        self, arY, arX, labels=("Longitudinal Position [m]", "Horizontal Position [m]")
    ):  # 1D plot on irregular mesh
        fig = self._pl.figure()
    ...
    def uti_plot2d1d(self, data, x_range, y_range, xc, yc, labels, _graphs_joined=True):
        ...
        if _graphs_joined:
            ...
            self._plot_2D(
                data, x_range, y_range, label2D, fig, 131
            )  # showing graphs in one panel
        else:
        ...
        xx = x0
        for ix in range(nx):
            arCutX[ix] = uti_math.interp_2d(
                xx, yc, x0, xStep, nx, y0, yStep, ny, data, inperpOrd, 1, 0
            )
            xx += xStep
        if _graphs_joined:
        ...
        for iy in range(ny):
            ...
            arCutY[iy] = uti_math.interp_2d(
                xc, yy, x0, xStep, nx, y0, yStep, ny, data, inperpOrd, 1, 0
            )
            yy += yStep
        if _graphs_joined:
            self._plot_1D(arCutY, y_range, label1V, fig, 133)
        else:
            self.uti_plot1d(arCutY, y_range, label1V)

        if _graphs_joined:
            self._pl.tight_layout()  # OC081115
    ...
    def uti_plot_data_file(
        ...
        data, mode, allrange, arLabels, arUnits = uti_plot_com.file_load(
            _fname, _read_labels, _multicolumn_data
        )  # MR31102017
        ...
        if _scale != "linear":
            ...
            if _scale not in available_scales:
                raise ValueError(
                    'Scale "{}" is not supported. Available scales: {}.'.format(
                        _scale, ", ".join(available_scales)
                    )
                )
            data[np.where(data <= 0.0)] = 1.0e-23
            data = getattr(np, _scale)(data)
        if _width_pixels:
            try:
                from scipy.ndimage import zoom
        ...
        if mode == m.T:
            ...
            fig = self.__mode_HV(
                data, allrange, arLabels, arUnits, _x, _y, _graphs_joined
            )
        # elif mode==m.EV:
        ...
        #  fig = __mode_EH(data,allrange)
            ...
            fig = self.__mode_EHV(
                data, allrange, arLabels, arUnits, _e, _x, _y, _graphs_joined
            )
    ...
    def uti_data_file_plot(
        ...
        return self.uti_plot_data_file(
            _fname,
            _read_labels,
            _e,
            _x,
    ...
    def _plot_1D_XvsY(
        self, data, column_x, column_y, figsize=(8, 5), typ=111
    ):  # MR31102017
    ...
    def _plot_1D_ir(self, ary, arx, labels, fig, typ=111):  # OC15112017
        if isinstance(arx, (list, array)):
            arx = np.array(arx)
        if isinstance(ary, (list, array)):
            ary = np.array(ary)
    ...
    def _plot_1D(self, ar1d, x_range, labels, fig, typ=111):
        ...
        if lenAr1d > x_range[2]:
            ar1d = np.array(ar1d[0 : x_range[2]])
        elif lenAr1d < x_range[2]:
            auxAr = array("d", [0] * x_range[2])
            for i in range(lenAr1d):
                auxAr[i] = ar1d[i]
            ar1d = np.array(auxAr)

        if isinstance(ar1d, (list, array)):
            ar1d = np.array(ar1d)
    ...
    def __mode_T(self, data, allrange, _ar_labels, _ar_units, _ec=0, _xc=0, _yc=0):
        ...
        toprint = (
            ...
            data[0],
            units[3],
        )  # squeeze have reduced it to an array with one element.
    ...
    def __mode_HV(
        self, data, allrange, _ar_labels, _ar_units, _xc=0, _yc=0, _graphs_joined=True
    ):
        ...
        return self.uti_plot2d1d(
            data, range_x, range_y, xc, yc, [label2D, label1H, label1V], _graphs_joined
        )
    ...
    def __mode_EHV(
        self, data, allrange, _ar_labels, _ar_units, _ec, _xc, _yc, _graphs_joined=1
    ):
        # allrange, units = _rescale_range(allrange)
        # allrange, units = _rescale_range(allrange, _ar_units, _ec, _xc, _yc)
        allrange, units = uti_plot_com.rescale_range(allrange, _ar_units, _ec, _xc, _yc)
        ...
        if _graphs_joined:
            ...
            self._plot_2D(
                arCutXY, range_x, range_y, label2D, fig, 221
            )  # showing graphs in one figure
