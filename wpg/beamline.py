...
class Beamline(object):
    ...
    def __init__(self, srwl_beamline=None):
        ...
        self.propagation_options = [
            {"optical_elements": [], "propagation_parameters": []}
        ]
    ...
    def append(self, optical_element, propagation_parameters):
        ...
        if not len(last_pp_opt["optical_elements"]) == len(
            last_pp_opt["propagation_parameters"]
        ):
            self.propagation_options.append(
                {"optical_elements": [], "propagation_parameters": []}
            )
            last_pp_opt = self.propagation_options[-1]

        if not all(
            [isinstance(o, srwlib.SRWLOpt) for o in last_pp_opt["optical_elements"]]
        ):
            self.propagation_options.append(
                {"optical_elements": [], "propagation_parameters": []}
            )
        ...
        if optical_element == [] or isinstance(
            optical_element, wpg.optical_elements.Empty
        ):
            pp = _get_srw_pp(propagation_parameters)
