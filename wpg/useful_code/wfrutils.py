...
def get_mesh(mwf):
    ...
    nx = wf_mesh.nx
    ny = wf_mesh.ny
    [xmin, xmax, ymin, ymax] = [wf_mesh.xMin, wf_mesh.xMax, wf_mesh.yMin, wf_mesh.yMax]
...
def show_slices_hsv(wf, slice_numbers=None, pretitle=""):
    ...
    if wf.params.wDomain == "time":
        pylab.plot(
            numpy.linspace(
                wf.params.Mesh.sliceMin, wf.params.Mesh.sliceMax, wf.params.Mesh.nSlices
            )
            * 1e15,
            intense,
        )
        ...
        pylab.plot(
            (slice_numbers * dt - wf.params.Mesh.nSlices * dt / 2)
            / wf.params.photonEnergy
            * 1e3,
            intense[slice_numbers],
            color="g",
...
def calculate_mediane(dd):
    ...
    x_axis = dd[:, 0]
    mediane = (
        numpy.max(x_axis[numpy.where(irr_x >= irr_max / 2)])
...
def propagate_run(ifname, ofname, optBL, bSaved=False):
    ...
    print("done")
    print("propagation lasted:", round((time.time() - startTime) / 6.0) / 10.0, "min")
    return wfr
