...
def show_diagnostics(prop_out_number):
    ...
    plt.figure()
    plt.imshow(
        data * dx * dy * 1e6 * J2eV / wf.params.photonEnergy,
        extent=[mesh.xMin * 1e6, mesh.xMax * 1e6, mesh.yMin * 1e6, mesh.yMax * 1e6],
    )
    ...
    temporal_struct = wf.custom_fields["history"]["parent"]["parent"]["misc"][
        "temporal_struct"
    ]
    ...
    plt.figure()
    plt.plot(
        temporal_struct[:, 0] - t0, temporal_struct[:, 1] * 1e-9, "b", label="original"
    )
    plt.hold(True)
    ...
    title = "The propagated pulse energy %.2f %s " % (
        pulse_energy.sum(axis=0) * dx * dy * 1e6 * dt * 1e3,
        "mJ",
    )
    plt.title(title)
    plt.xlabel("time [fs]")
...
def main():
    ...
    parser.add_option(
        "--input-file",
        dest="in_fname",
        default="prop_out_1.h5",
        help="Input wavefront file: prop_out_***.h5",
    )
