...
class ReferenceTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        if "FELsource_5keV_9fs_slicing12_3-29fs_104x104.h5" not in os.listdir("."):
            try:
                cls.__fel_source = wgetData(
                    url="https://docs.xfel.eu/alfresco/d/a/workspace/SpacesStore/343db429-a2df-4ae9-ac2e-c289a34c6fc1/FELsource_5keV_9fs_slicing12_3-29fs_104x104.h5"
                )
            except:
                raise RuntimeError(
                    "Unable to download test data. Please try again later. If problem persists, contact support."
                )
                sys.exit()
        else:
        ...
        if "reference_prop_out.h5" not in os.listdir("."):
            try:
                cls.__reference_prop_out = wgetData(
                    url="https://docs.xfel.eu/alfresco/d/a/workspace/SpacesStore/9b263892-1cb0-42e3-b05a-7507f66c67db/reference_prop_out.h5"
                )
            except:
                raise RuntimeError(
                    "Unable to download test data. Please try again later. If problem persists, contact support."
    ...
    def test9fsKB(self):
        ...
        Ehor_reference = h5py.File(self.__reference_prop_out, "r")[
            "data/arrEhor"
        ].value.flatten()
