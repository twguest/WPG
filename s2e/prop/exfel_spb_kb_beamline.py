...
def get_beamline():
    ...
    # Mirror profiles.
    wf_dist_om = optical_elements.Mirror_plane(
        orient="x",
        theta=theta_om,
    ...
    wf_dist_hfm = optical_elements.Mirror_plane(
        orient="x",
        theta=theta_kb,
    ...
    wf_dist_vfm = optical_elements.Mirror_plane(
        orient="y",
        theta=theta_kb,
        length=kb_mirror_length,
        range_xy=kb_clear_ap,
    ...
    bl0.append(ap0, Use_PP(semi_analytical_treatment=0, zoom=1.0, sampling=1.0 / 0.4))
    #### Increase sampling again, reduce ROI => ROI ~10 fwhm, 700x700 sampling.
    bl0.append(
        drift0, Use_PP(semi_analytical_treatment=1, zoom=0.5, sampling=1.5 / 0.28)
    )
