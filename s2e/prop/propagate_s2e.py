...
MIRROR_DATA_DIR = os.path.join(
    os.path.abspath(os.path.dirname(__file__)), "data_common/"
)
        ...
        with h5py.File(history_file_name) as history_h5:
            ...
            for k in history_h5:
                if k == "history":
                    ...
                    wf_h5["history"]["parent"]["detail"]["data"] = h5py.ExternalLink(
                        history_file_name, "/data"
                    )
...
def get_intensity_on_axis(wfr):
    ...
    sz[:, 1] = wf_intensity[mesh.ny // 2, mesh.nx // 2, :] / wf_intensity.max()
...
def main():
    ...
    parser.add_argument(
        "--input-directory",
        dest="in_dname",
        help="Input directory with wavefront files",
    )
    parser.add_argument(
        "--output-directory",
        dest="out_dname",
        help="Output directory with wavefront files",
    )
    parser.add_argument(
        "-n",
        "--cpu-number",
        dest="cpu_number",
        default=int((multiprocessing.cpu_count() + 1) / 2),
        help="Number of cores for batch wavefronts propagation, default value NUMBER_OF_CPU/2",
    )
    parser.add_argument(
        "--beamline-file",
        dest="beamline_file",
        help="Python file with beamline description",
    )
    ...
    if not (options.in_fname or options.in_dname):  # if filename is not given
        parser.error(
            "Input filename or directiry not specified, use --input-file or --input-directory options"
        )
        return

    if not (options.out_fname or options.out_dname):  # if filename is not given
        parser.error(
            "Output filename or directiry not specified, use --output-file or --output-directory options"
        )
        return
    ...
    if options.in_dname and options.out_dname:
        print(
            "Input directory {}, output directory {}, number of cores {}".format(
                options.in_dname, options.out_dname, options.cpu_number
            )
        )
        print("Batch propagation started")
        directory_process(
            options.in_dname, options.out_dname, get_beamline, int(options.cpu_number)
        )
        ...
        print(
            "Input file {}, output file {}".format(
                options.in_fname, options.out_fname, get_beamline
            )
        )
