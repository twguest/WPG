...
def get_beamline():
    ...
    hom1_aperture = Aperture(
        shape="r", ap_or_ob="a", Dx=om_clear_ap, Dy=om_clear_ap / aperture_x_to_y_ratio
    )

    # Append to beamline.
    beamline.append(
        hom1_aperture, Use_PP(semi_analytical_treatment=0, zoom=zoom, sampling=zoom)
    )
    ...
    beamline.append(
        hom2_aperture, Use_PP(semi_analytical_treatment=0, zoom=zoom, sampling=zoom)
    )
    ...
    crl_front_aperture = Aperture(
        "c", "a", crl_front_aperture_diameter, crl_front_aperture_diameter
    )

    ### Define CRL
    crl_focussing_plane = 3  # Both horizontal and vertical.
    crl_delta = (
        4.8308e-06  # Refractive index decrement (n = 1- delta - i*beta) @ 8.4 keV
    )
    ...
    beamline.append(
        crl_to_exp_drift, Use_PP(semi_analytical_treatment=1, zoom=1, sampling=1)
    )
