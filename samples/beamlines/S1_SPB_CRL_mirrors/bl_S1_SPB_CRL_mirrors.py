def get_beamline():
    ...
    hom1_wavefront_distortion = Mirror_plane(
        orient="x",
        theta=theta_om,
    ...
    bl0.append(
        hom2, Use_PP(semi_analytical_treatment=0, zoom=zoom, sampling=zoom / 0.75)
    )

    # define and apply mirror distortion.
    hom2_wavefront_distortion = Mirror_plane(
        orient="x",
        theta=theta_om,
        length=om_mirror_length,
        range_xy=om_clear_ap / aperture_x_to_y_ratio,
        filename=os.path.join(mirrors_path, "mirror2.dat"),
        scale=1.0,
        xscale=1.0,
        bPlot=True,
    )
